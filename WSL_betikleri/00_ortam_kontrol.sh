...
cat <<'NOT'
PROJECT\fastq files klasorundeki dosyalar HAM OKUMA DEGIL.
the colleague's sequence_extraction.sh betigi her ornekten yalnizca en bol ILK BES
...
PT=""
for c in "/path/to/your/project" "/path/to/your/project"; do
  [ -d "$c" ] && PT="$c" && break
done
if [ -n "$PT" ]; then
  echo "PROJECT: $PT"
...
if lens:
    ...
    print("NOT: a colleague database300mers.kmer_distrib kullanmis. Medyan bunun cok")
...
PY
  ...
  echo "PROJECT klasoru /mnt/c altinda bulunamadi, yolu elle bildirin."
fi
