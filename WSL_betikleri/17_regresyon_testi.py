...
Kullanım:
  python3 17_regresyon_testi.py                 # hızlı testler
  python3 17_regresyon_testi.py --gercek-veri   # gerçek konsensüs ve okumalarla
"""
...
# =======================================================================
def testler(a):
    E = yukle("E", "03_primer_aday_uret.py")
    G = yukle("G", "04_grup_primer.py")
    O = yukle("O", "09_ozgulluk.py")
    K = yukle("K", "12_baskin_alel_konsensus.py")
    D = yukle("D", "10_ayirt_edilemez.py")
    ...
    k09 = open(os.path.join(HERE, "09_ozgulluk.py"), encoding="utf-8").read()
    k08 = open(os.path.join(HERE, "08_toplu_tasarim.py"), encoding="utf-8").read()
    ...
    DV = yukle("DV", "14_dis_veritabani.py")
    MF = yukle("MF", "19_mfeprimer.py")
    ...
    if shutil.which("blastn") and shutil.which("makeblastdb"):
        ...
        subprocess.run(
            [sys.executable, os.path.join(HERE, "14_dis_veritabani.py"),
    ...
    DZ = yukle("DZ", "27_duzey_denetimi.py")
    ...
    if shutil.which("blastn") and shutil.which("makeblastdb"):
        ...
        r3 = subprocess.run(
            [sys.executable, os.path.join(HERE, "27_duzey_denetimi.py"),
    ...
    RT = yukle("RT", "15_referans_tasarim.py")
            ...
            for t in tsvler[:6]:
                r = subprocess.run(
                    [sys.executable, os.path.join(HERE,
                                                  "16_primer_geometri_denetimi.py"),
