...
"""
08_toplu_tasarim.py
hedefler.tsv dosyasındaki bütün toplantı kararlarını 04_grup_primer.py ile
...
MOTOR = os.path.join(HERE, "04_grup_primer.py")


def _ayirt_modulu():
    """10_ayirt_edilemez.py'yi tek kaynak olarak yukler."""
    yol = os.path.join(HERE, "10_ayirt_edilemez.py")
...
def _girdi_parmak_izi(a):
    ...
    for f in sorted(glob.glob(os.path.join(a.kons, "*_konsensus.fasta"))):
        try:
            ...
            pass
    for yol in (a.hedefler, MOTOR, os.path.join(HERE, "03_primer_aday_uret.py"),
                os.path.join(HERE, "10_ayirt_edilemez.py")):
        try:
            st = os.stat(yol)
...
def main():
    ...
    if a.ayirt_edilemez_cikar and AYIRT is not None:
        ...
        log("UYARI: 10_ayirt_edilemez.py bulunamadi, ayirt edilemez kutu "
