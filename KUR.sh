...
#     SRC/WSL/01_kurulum.sh          (araclar)
#     SRC/WSL/87_MFEPRIMER_KUR.sh    (MFEprimer)
#     SRC/primerscope/reference/download_refseq.py  (RefSeq)
...
#     veritabaniyla calisirsiniz" (SRC/WSL/01_kurulum.sh). Bu yuzden her indirme
...
komut_durum() {
  ...
  if [ -n "$bulunan" ]; then
    while read -r h; do
      ...
      [ -f "$d/opts.k2d" ] && bilgi "    k-mer bilgisi icin: bash SRC/WSL/130_KRAKEN_ARAC.sh vt-kimlik"
    done <<< "$bulunan"
  else
...
komut_araclar() {
  ...
  if ! command -v mfeprimer >/dev/null 2>&1 \
    ...
    bilgi "MFEprimer icin: bash SRC/WSL/87_MFEPRIMER_KUR.sh"
    bilgi "  (surum adresi surekli degistigi icin ayri betikte tutuluyor)"
    EKSIK+=("mfeprimer (SRC/WSL/87_MFEPRIMER_KUR.sh ile kurun)")
  fi
...
komut_kraken_indir() {
  ...
  bilgi "   bash SRC/WSL/130_KRAKEN_ARAC.sh vt-kimlik"
}
...
komut_kraken_kur() {
  ...
  cat <<EOF
   ...
   (SRC/0_TESLIM_HOCA/KRAKEN_KARSILASTIRMA.md). Sebep k-mer degil, LCA'nin
   ...
   ------------------------------------------------
       ...
       bash SRC/WSL/130_KRAKEN_ARAC.sh esik      # esik taramasi
       bash SRC/WSL/130_KRAKEN_ARAC.sh tablo     # dort sutunlu karsilastirma
EOF
