...
#     ALI/WSL/01_kurulum.sh          (araclar)
#     ALI/WSL/87_MFEPRIMER_KUR.sh    (MFEprimer)
#     ALI/primerscope/reference/download_refseq.py  (RefSeq)
#     QIIME2_PICRUST_KOSU.bat        (QIIME2 + PICRUSt2 ortami)
#     INDEKS_KUR.sh                  (MFEprimer indeksi - HALA AYRI, buradan cagrilir)
#
#  KULLANIM
#  --------
#     bash KUR.sh durum                 # neyin kurulu oldugunu OLCER, hicbir sey degistirmez
#     bash KUR.sh araclar               # kraken2, bracken, minimap2, samtools, blast, seqkit, mfeprimer
#     bash KUR.sh veritabani            # SILVA + UNITE + PR2 + ROD + RefSeq (kimlik icin 12 kaynak)
#     bash KUR.sh veritabani --yalniz refseq,pr2
#     bash KUR.sh kraken-indir          # hazir Kraken2 veritabani (k-mer sabit: k=35)
#     bash KUR.sh kraken-kur --kmer 31  # KENDI Kraken2 veritabanini KENDI k-mer'inle kur
#     bash KUR.sh qiime                 # QIIME2 + PICRUSt2 + SILVA siniflandirici
#     bash KUR.sh hepsi                 # araclar + veritabani + qiime  (kraken AYRI, secim ister)
#
#  UC TASARIM KARARI
#  -----------------
#  1) INDIRILEN DOSYA DOGRULANIR, ADRESE GUVENILMEZ.
#     Bu projenin kurali: "koda gomulen adres sessizce eskir ve yanlis
#     veritabaniyla calisirsiniz" (ALI/WSL/01_kurulum.sh). Bu yuzden her indirme
...
ortam_denetimi() {
  ...
  bilgi "Dogrusu:"
  bilgi "   wsl bash -lc \"cd '$KOK' && bash KUR.sh $*\""
  printf '\n'
}
...
#     bash KUR.sh veritabani --yalniz silva_ssu --url https://.../BASKA.fasta.gz
#
...
# hedef dosya adlari - kod bu adlari bekliyor (KURTARMA/kimlik_dogrulama.py VTB listesi)
declare -A HEDEF=(
...
fasta_dogrula() {
  ...
  # 3^9 k-mer girer. MFEprimer bu durumda HATA VERMEZ. (bkz. INDEKS_KUR.sh)
  local ornek u t
  ornek=$(grep -v '^>' "$f" 2>/dev/null | head -2000 | tr -d '\n')
  u=$(printf '%s' "$ornek" | tr -cd 'Uu' | wc -c)
  t=$(printf '%s' "$ornek" | tr -cd 'Tt' | wc -c)
  if [ "$u" -gt "$t" ]; then
    bilgi "$(basename "$f"): $n kayit, RNA alfabesi (U>T) - INDEKS_KUR.sh U->T donusumu yapacak"
  else
...
indir_ve_dogrula() {
  ...
  if ! wget -c -q --show-progress -O "$gz" "$url"; then
    ...
    bilgi "    bash KUR.sh veritabani --yalniz $anahtar --url <YENI_ADRES>"
...
komut_durum() {
  ...
  if [ -n "$bulunan" ]; then
    while read -r h; do
      ...
      [ -f "$d/opts.k2d" ] && bilgi "    k-mer bilgisi icin: bash ALI/WSL/130_KRAKEN_ARAC.sh vt-kimlik"
    done <<< "$bulunan"
  else
    printf '   \033[31mYOK\033[0m  -> bash KUR.sh kraken-indir   ya da   bash KUR.sh kraken-kur\n'
  fi
  ...
  bilgi "KURTARMA/kimlik_dogrulama.py bir iddiayi DOGRULANDI saymak icin EN AZ IKI"
...
komut_araclar() {
  ...
  if ! command -v mfeprimer >/dev/null 2>&1 \
     && [ ! -x "$KOK/ARACLAR/mfeprimer" ]; then
    bilgi "MFEprimer icin: bash ALI/WSL/87_MFEPRIMER_KUR.sh"
    bilgi "  (surum adresi surekli degistigi icin ayri betikte tutuluyor)"
    EKSIK+=("mfeprimer (ALI/WSL/87_MFEPRIMER_KUR.sh ile kurun)")
  fi
...
komut_veritabani() {
  ...
  if [ -z "$yalniz" ] || [[ ",$yalniz," == *",unite,"* ]]; then
    if [ ! -s "$REFDB/UNITE_ITS.fasta" ]; then
      ...
      bilgi "2) bash KUR.sh veritabani --yalniz unite --url <KOPYALADIGINIZ_ADRES>"
  ...
  bilgi "   bash INDEKS_KUR.sh --liste          # aday dosyalari gorun"
  bilgi "   bash INDEKS_KUR.sh <dosya_adi>      # tek tek kurun"
...
komut_kraken_indir() {
  ...
  uyar "SECEMEZSINIZ. Kendi k-mer'inizi istiyorsaniz: bash KUR.sh kraken-kur"
  bilgi
  bilgi "Kurduktan sonra kimligini DOGRULAYIN (hangi surum, hangi k-mer):"
  bilgi "   bash ALI/WSL/130_KRAKEN_ARAC.sh vt-kimlik"
}
...
komut_kraken_kur() {
  ...
  cat <<EOF
   ...
   ------------------------------------------------
       bash KUR.sh kraken-kur --kmer 35 --db ~/k2db_k35
       bash KUR.sh kraken-kur --kmer 31 --db ~/k2db_k31
       bash ALI/WSL/130_KRAKEN_ARAC.sh esik      # esik taramasi
       bash ALI/WSL/130_KRAKEN_ARAC.sh tablo     # dort sutunlu karsilastirma
EOF
  if ! command -v kraken2-build >/dev/null 2>&1; then
    hata "kraken2-build bulunamadi. Once: bash KUR.sh araclar"
...
ozet() {
  ...
  bilgi "durumu gormek icin: bash KUR.sh durum"
  bilgi "log: $LOG"
  return 0
}
...
case "$KOMUT" in
  ...
  hepsi)         komut_araclar; komut_veritabani; komut_qiime
                 ...
                 bilgi "   bash KUR.sh kraken-indir     # hazir, k=35 sabit"
                 bilgi "   bash KUR.sh kraken-kur --kmer 31"
                 ozet ;;
  *)
