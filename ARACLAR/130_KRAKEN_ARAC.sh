...
#
# THE COLLEAGUE'S BETIKLERINDEN NE ALINDI
...
#
# TUSLAR
#   bash 130_KRAKEN_ARAC.sh bellek-ayari  .wslconfig onerir (KILITLENME COZUMU, ONCE BUNU)
#   bash 130_KRAKEN_ARAC.sh dogrula-ornek ornek tam veriyi temsil ediyor mu
#   bash 130_KRAKEN_ARAC.sh kraken-yol   kraken2'nin cozulmus tam yolunu basar (makine okunur)
#   bash 130_KRAKEN_ARAC.sh durum        ortam ve veritabani denetimi, hicbir sey kosmaz
#   bash 130_KRAKEN_ARAC.sh vt-ara       diskte kraken2 veritabani arar ve listeler
#   bash 130_KRAKEN_ARAC.sh vt-kimlik    veritabani hangi surum, derin tespit
#   bash 130_KRAKEN_ARAC.sh src-vt       a colleague hangi veritabanini kullandi, kanittan cikarim
#   bash 130_KRAKEN_ARAC.sh sinav        butun selftestler
#   bash 130_KRAKEN_ARAC.sh sure         kucuk denemeyle GERCEK hiz olcumu ve sure tahmini
#   bash 130_KRAKEN_ARAC.sh esik-a       guven esigi taramasi, VT_A uzerinde
#   bash 130_KRAKEN_ARAC.sh esik-b       guven esigi taramasi, VT_B uzerinde
#   bash 130_KRAKEN_ARAC.sh esik         ikisi de, sonra yan yana egri
#   bash 130_KRAKEN_ARAC.sh tablo        dort sutunlu karsilastirma tablosu
#   bash 130_KRAKEN_ARAC.sh hepsi        sinav + esik + tablo
#   bash 130_KRAKEN_ARAC.sh ozelvt-kur   ozel veritabani kurulumu (EN SON CARE)
#   bash 130_KRAKEN_ARAC.sh ozelvt-kos   ozel veritabaniyla kosu
#
# DEGISKENLER
...
# --- SRC klasoru olculur, tahmin edilmez (86_KRAKEN_YENIDEN.sh ile ayni yol) ---
_BETIK_DIZIN="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SRC="${SRC:-$(cd "$_BETIK_DIZIN/.." && pwd)}"
if [ ! -d "$SRC/0_TESLIM_HOCA" ]; then
  echo "HATA: SRC klasoru dogrulanamadi ('$SRC' icinde 0_TESLIM_HOCA yok)."
  echo "  Betik konumu: $_BETIK_DIZIN"
  echo "  Elle vermek icin:  SRC=/tam/yol/SRC bash $0 $TUS"
  exit 1
fi
...
KAYNAK="$SRC/SONUCLAR/fastq files"
IS_A="$SRC/SONUCLAR/kraken_esik_A"
IS_B="$SRC/SONUCLAR/kraken_esik_B"
OZEL_IS="$SRC/SONUCLAR/kraken_ozelvt"

log_ac() {
  local ad="$1"
  mkdir -p "$SRC/WSL/loglar"
  LOG="$SRC/WSL/loglar/130_${ad}_$(date '+%Y%m%d_%H%M%S').log"
  exec > >(tee -a "$LOG") 2>&1
  echo "=============================================================="
  echo "130_KRAKEN_ARAC  tus: $ad  baslangic $(date '+%Y-%m-%d %H:%M:%S %Z')"
...
# Bu fonksiyon the colleague's 86_KRAKEN_YENIDEN.sh ve 02_kraken_bracken.sh betiklerinden
...
#   1) Zaten PATH'te mi.
#   2) THE COLLEAGUE'S YOLU - degistirilmedi, aynen korundu: $HOME/bin PATH'e eklenir,
...
ortam_ac() {
  ...
  for k in \
    ...
    # (the colleague's kraken2_driver.sh betigi kraken2'yi cipl ak cagiriyordu; biz
  ...
  # --- 3) THE COLLEAGUE'S YOLU: ortami etkinlestir -------------------------------
...
tus_kraken_yol() {
  ...
  fi
  {
    ...
    echo "    bash $SRC/WSL/01_kurulum.sh"
...
vt_surum() {
  ...
  # --- olcum 1: inspect ---
  local o1="olculemedi"
  local ins="$SRC/SONUCLAR/vt_inspect_$(basename "$d").txt"
  ...
  local pi="$SRC/SONUCLAR/vt_parmak_izi.tsv"
  mkdir -p "$(dirname "$pi")"
...
eski_kosu_tespit() {
  ...
  local bulundu=0
  for l in "$SRC"/WSL/loglar/*.log; do
    [ -f "$l" ] || continue
    local yol boy
  ...
  for r in "$SRC/SONUCLAR/kraken_yeniden/tum.report" \
           "$SRC/SONUCLAR/kraken results"/*/*_kraken2.report; do
    [ -f "$r" ] || continue
    local m p b
  ...
  local ilk="$SRC/SONUCLAR/kraken_yeniden/tum.report"
  if [ -f "$ilk" ]; then
    local m p b
...
# =========================================================================
# TUS: src-vt   SRC HANGI VERITABANINI KULLANDI
...
tus_ali_vt() {
  ...
  set +e
  echo "======================================================================"
  echo "SRC HANGI VERITABANINI KULLANDI"
  echo "======================================================================"
  echo
  echo "KANIT 1: the colleague's kendi betiklerindeki --db argumani"
  local k1=""
  for f in "$SRC/troubleshooting tools"/*.sh; do
    [ -f "$f" ] || continue
    grep -HnoE "(KRAKEN[A-Z0-9_]*(DB|PATH|FOLDER)|--db)[= ]*\"?[^\" ]+" "$f" 2>/dev/null \
      | sed 's|^.*/||; s/^/    /' || true
  done
  k1=$(grep -rhoE "/[^\" ]*(pluspf|pluspfp|standard|k2[a-z0-9_]*)[^\" ]*" \
        "$SRC/troubleshooting tools"/*.sh 2>/dev/null | sort -u | head -3)
  if [ -n "$k1" ]; then
    echo
  ...
  [ "$k2" -eq 1 ] || echo "    the colleague's kosusuna ait log yok (loglar bizim kosularimiza ait)"

  echo
  echo "KANIT 3: rapor icerigi (hangi alanlar veritabaninda VARDI)"
  local r="$SRC/SONUCLAR/kraken results/A1/edited_barcode01_kraken2.report"
  if [ -f "$r" ]; then
    local m p b v
    ...
    echo "    Yani the colleague's veritabani en az PlusPF kapsamindadir, Standard degil."
    ...
    echo "    the colleague's raporu bulunamadi: $r"
  fi

  echo
  echo "KANIT 4: bizim yeniden kosumuz the colleague's sonucunu tekrarladi mi"
  local oz="$SRC/SONUCLAR/kraken_yeniden/kraken_ozet.csv"
  if [ -f "$oz" ]; then
    local top uy ay
    ...
    echo "    the colleague's ciktisina gore ayrildigina gore, veritabani AYNI olsaydi"
    echo "    butun kutular kendi etiketini tekrarlardi."
    if [ "$ay" -gt 0 ]; then
      echo "    $ay kutu tekrarlamadi, dolayisiyla bizim yeniden kosumuzun"
      echo "    veritabani the colleague's veritabaniyla AYNI DEGILDIR."
      echo
  ...
  echo "======================================================================"
  if printf '%s' "$k1" | grep -qi "pluspf16\|pluspf_16\|_16gb"; then
    echo "  the colleague's veritabani: PlusPF-16 (PlusPF'in 16 GB'a kapakli surumu),"
...
birlestir() {
  local hedef="$1"
  local eski="$SRC/SONUCLAR/kraken_yeniden/tum.fastq"
  if [ -s "$hedef" ]; then
  ...
  if [ -s "$eski" ]; then
    ...
    cp "$eski" "$hedef"
    cp "$SRC/SONUCLAR/kraken_yeniden/kaynak_sayim.tsv" "$(dirname "$hedef")/" 2>/dev/null || true
...
tus_sinav() {
  ...
  local hata=0
  for p in 131_ESIK_OZET.py 133_KARSILASTIRMA.py 132_OZEL_TAKSONOMI.py 86_KRAKEN_OZET.py; do
    echo; echo "--- $p"
...
tus_esik() {
  ...
  if [ "$ikili" -eq 1 ]; then
    ...
    echo
    python3 "$_BETIK_DIZIN/131_ESIK_OZET.py" --src "$SRC" \
...
# 133_KARSILASTIRMA.py eksik girdiye dayaniklidir: PlusPFP kosusu yoksa 3.
...
tus_ozelvt_kur() {
  ...
  [ "$var" -gt 0 ] || { echo; echo "HATA: hicbir referans kumesi bulunamadi ($SRC/REFERANS_DB)"; exit 1; }
  ...
  python3 "$_BETIK_DIZIN/132_OZEL_TAKSONOMI.py" --cikti "$OZELVT" "${args[@]}"
...
kume_dosya() {
  case "$1" in
    silva_ssu) echo "$SRC/REFERANS_DB/SILVA_138.2_SSURef_NR99.fasta" ;;
    silva_lsu) echo "$SRC/REFERANS_DB/SILVA_138.2_LSURef_NR99.fasta" ;;
    unite)     echo "$SRC/REFERANS_DB/UNITE_ITS.fasta" ;;
    pr2)       echo "$SRC/REFERANS_DB/PR2_SSU_taxo_long.fasta" ;;
    rod)       echo "$SRC/REFERANS_DB/ROD_v1.2_operon_variants.fasta" ;;
    *) return 1 ;;
  esac
}
...
tus_ozelvt_kos() {
  ...
  python3 "$_BETIK_DIZIN/86_KRAKEN_OZET.py" --is "$OZEL_IS" --src "$SRC" || true
}
...
autoMemoryReclaim=gradual"
  ...
  local CIKTI="$SRC/wslconfig_ONERILEN.txt"
  printf '%s\n' "$ICERIK" > "$CIKTI"
...
tus_dogrula_ornek() {
  ...
  awk -v s="$sapma_max" 'BEGIN{
    if (s <= 2.0) {
      ...
      print "  ORNEK degerini buyutup tekrarlayin, orn: ORNEK=300000 bash 130_KRAKEN_ARAC.sh esik"
    }
  }'
...
case "$TUS" in
  ...
  esik-a)      tus_sinav >/dev/null || { echo "SINAV KALDI. Ayrinti: bash $0 sinav"; exit 2; }
               ...
               python3 "$_BETIK_DIZIN/131_ESIK_OZET.py" --src "$SRC" --is "$IS_A" --ad "$(basename "$VT_A")" ;;
  esik-b)      [ -n "$VT_B" ] || { echo "HATA: VT_B verilmedi.  VT_B=/yol bash $0 esik-b"; exit 1; }
               ...
               python3 "$_BETIK_DIZIN/131_ESIK_OZET.py" --src "$SRC" --is "$IS_B" --ad "$(basename "$VT_B")" ;;
  ...
  *)           sed -n '3,48p' "$0" | sed 's/^# \{0,1\}//'
               echo; echo "Ayrinti icin: $SRC/NASIL_DEVAM_EDILIR.md" ;;
esac
