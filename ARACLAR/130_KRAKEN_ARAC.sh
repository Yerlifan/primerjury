...
#
# ALI'NIN BETIKLERINDEN NE ALINDI
...
#   bash 130_KRAKEN_ARAC.sh ali-vt       Ali hangi veritabanini kullandi, kanittan cikarim
...
# --- ALI klasoru olculur, tahmin edilmez (86_KRAKEN_YENIDEN.sh ile ayni yol) ---
_BETIK_DIZIN="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ALI="${ALI:-$(cd "$_BETIK_DIZIN/.." && pwd)}"
if [ ! -d "$ALI/0_TESLIM_HOCA" ]; then
  echo "HATA: ALI klasoru dogrulanamadi ('$ALI' icinde 0_TESLIM_HOCA yok)."
  echo "  Betik konumu: $_BETIK_DIZIN"
  echo "  Elle vermek icin:  ALI=/tam/yol/ALI bash $0 $TUS"
  exit 1
fi
...
KAYNAK="$ALI/SONUCLAR/fastq files"
IS_A="$ALI/SONUCLAR/kraken_esik_A"
IS_B="$ALI/SONUCLAR/kraken_esik_B"
OZEL_IS="$ALI/SONUCLAR/kraken_ozelvt"

log_ac() {
  local ad="$1"
  mkdir -p "$ALI/WSL/loglar"
  LOG="$ALI/WSL/loglar/130_${ad}_$(date '+%Y%m%d_%H%M%S').log"
  exec > >(tee -a "$LOG") 2>&1
  echo "=============================================================="
...
# Bu fonksiyon Ali'nin 86_KRAKEN_YENIDEN.sh ve 02_kraken_bracken.sh betiklerinden
...
#   1) Zaten PATH'te mi.
#   2) ALI'NIN YOLU - degistirilmedi, aynen korundu: $HOME/bin PATH'e eklenir,
...
ortam_ac() {
  ...
  for k in \
    ...
    # (Ali'nin kraken2_driver.sh betigi kraken2'yi cipl ak cagiriyordu; biz
  ...
  # --- 3) ALI'NIN YOLU: ortami etkinlestir -------------------------------
...
tus_kraken_yol() {
  ...
  fi
  {
    ...
    echo "    bash $ALI/WSL/01_kurulum.sh"
...
vt_surum() {
  ...
  # --- olcum 1: inspect ---
  local o1="olculemedi"
  local ins="$ALI/SONUCLAR/vt_inspect_$(basename "$d").txt"
  ...
  local pi="$ALI/SONUCLAR/vt_parmak_izi.tsv"
  mkdir -p "$(dirname "$pi")"
...
eski_kosu_tespit() {
  ...
  local bulundu=0
  for l in "$ALI"/WSL/loglar/*.log; do
    [ -f "$l" ] || continue
    local yol boy
  ...
  for r in "$ALI/SONUCLAR/kraken_yeniden/tum.report" \
           "$ALI/SONUCLAR/kraken results"/*/*_kraken2.report; do
    [ -f "$r" ] || continue
    local m p b
  ...
  local ilk="$ALI/SONUCLAR/kraken_yeniden/tum.report"
  if [ -f "$ilk" ]; then
    local m p b
...
# =========================================================================
# TUS: ali-vt   ALI HANGI VERITABANINI KULLANDI
...
tus_ali_vt() {
  ...
  set +e
  echo "======================================================================"
  echo "ALI HANGI VERITABANINI KULLANDI"
  echo "======================================================================"
  echo
  echo "KANIT 1: Ali'nin kendi betiklerindeki --db argumani"
  local k1=""
  for f in "$ALI/troubleshooting tools"/*.sh; do
    [ -f "$f" ] || continue
    grep -HnoE "(KRAKEN[A-Z0-9_]*(DB|PATH|FOLDER)|--db)[= ]*\"?[^\" ]+" "$f" 2>/dev/null \
      | sed 's|^.*/||; s/^/    /' || true
  done
  k1=$(grep -rhoE "/[^\" ]*(pluspf|pluspfp|standard|k2[a-z0-9_]*)[^\" ]*" \
        "$ALI/troubleshooting tools"/*.sh 2>/dev/null | sort -u | head -3)
  if [ -n "$k1" ]; then
    echo
  ...
  [ "$k2" -eq 1 ] || echo "    Ali'nin kosusuna ait log yok (loglar bizim kosularimiza ait)"

  echo
  echo "KANIT 3: rapor icerigi (hangi alanlar veritabaninda VARDI)"
  local r="$ALI/SONUCLAR/kraken results/A1/edited_barcode01_kraken2.report"
  if [ -f "$r" ]; then
    local m p b v
    ...
    echo "    Yani Ali'nin veritabani en az PlusPF kapsamindadir, Standard degil."
    ...
    echo "    Ali'nin raporu bulunamadi: $r"
  fi

  echo
  echo "KANIT 4: bizim yeniden kosumuz Ali'nin sonucunu tekrarladi mi"
  local oz="$ALI/SONUCLAR/kraken_yeniden/kraken_ozet.csv"
  if [ -f "$oz" ]; then
    local top uy ay
    ...
    echo "    Ali'nin ciktisina gore ayrildigina gore, veritabani AYNI olsaydi"
    echo "    butun kutular kendi etiketini tekrarlardi."
    if [ "$ay" -gt 0 ]; then
      echo "    $ay kutu tekrarlamadi, dolayisiyla bizim yeniden kosumuzun"
      echo "    veritabani Ali'nin veritabaniyla AYNI DEGILDIR."
      echo
  ...
  echo "======================================================================"
  if printf '%s' "$k1" | grep -qi "pluspf16\|pluspf_16\|_16gb"; then
    echo "  Ali'nin veritabani: PlusPF-16 (PlusPF'in 16 GB'a kapakli surumu),"
...
birlestir() {
  local hedef="$1"
  local eski="$ALI/SONUCLAR/kraken_yeniden/tum.fastq"
  if [ -s "$hedef" ]; then
  ...
  if [ -s "$eski" ]; then
    ...
    cp "$eski" "$hedef"
    cp "$ALI/SONUCLAR/kraken_yeniden/kaynak_sayim.tsv" "$(dirname "$hedef")/" 2>/dev/null || true
...
tus_esik() {
  ...
  if [ "$ikili" -eq 1 ]; then
    ...
    echo
    python3 "$_BETIK_DIZIN/131_ESIK_OZET.py" --ali "$ALI" \
...
tus_ozelvt_kur() {
  ...
  [ "$var" -gt 0 ] || { echo; echo "HATA: hicbir referans kumesi bulunamadi ($ALI/REFERANS_DB)"; exit 1; }
...
kume_dosya() {
  case "$1" in
    silva_ssu) echo "$ALI/REFERANS_DB/SILVA_138.2_SSURef_NR99.fasta" ;;
    silva_lsu) echo "$ALI/REFERANS_DB/SILVA_138.2_LSURef_NR99.fasta" ;;
    unite)     echo "$ALI/REFERANS_DB/UNITE_ITS.fasta" ;;
    pr2)       echo "$ALI/REFERANS_DB/PR2_SSU_taxo_long.fasta" ;;
    rod)       echo "$ALI/REFERANS_DB/ROD_v1.2_operon_variants.fasta" ;;
    *) return 1 ;;
  esac
}
...
tus_ozelvt_kos() {
  ...
  python3 "$_BETIK_DIZIN/86_KRAKEN_OZET.py" --is "$OZEL_IS" --ali "$ALI" || true
}
...
autoMemoryReclaim=gradual"
  ...
  local CIKTI="$ALI/wslconfig_ONERILEN.txt"
  printf '%s\n' "$ICERIK" > "$CIKTI"
  ...
  echo "       C:\\Users\\yerli\\Masaustu\\ALI\\wslconfig_ONERILEN.txt"
  echo
...
case "$TUS" in
  ...
  esik-a)      tus_sinav >/dev/null || { echo "SINAV KALDI. Ayrinti: bash $0 sinav"; exit 2; }
               ...
               python3 "$_BETIK_DIZIN/131_ESIK_OZET.py" --ali "$ALI" --is "$IS_A" --ad "$(basename "$VT_A")" ;;
  esik-b)      [ -n "$VT_B" ] || { echo "HATA: VT_B verilmedi.  VT_B=/yol bash $0 esik-b"; exit 1; }
               ...
               python3 "$_BETIK_DIZIN/131_ESIK_OZET.py" --ali "$ALI" --is "$IS_B" --ad "$(basename "$VT_B")" ;;
  ...
  *)           sed -n '3,48p' "$0" | sed 's/^# \{0,1\}//'
               echo; echo "Ayrinti icin: $ALI/NASIL_DEVAM_EDILIR.md" ;;
esac
