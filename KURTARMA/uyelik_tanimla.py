...
"Kosulsuz" sozcugu yoneticiye "tartisilacak bir sey yok" diye okunur; oysa vardi.
...
gerekcesini satir satir yazar. Sonra:
  ...
  2) KAPSAMLI_ARAMA/hedef_uyelik.tsv dosyasina acik tanimi ekler
  3) istenirse panel kaynagina plaka/Ta satiri ekler
Her dosyanin once yedegini alir ve ne yaptigini basar.

Kosum (once PLAN):
  python KURTARMA/uyelik_tanimla.py --kok . --hedef Petriella_cinsi --kalip Petriella
...
def main():
    ...
    hy = os.path.join(kok, 'KAPSAMLI_ARAMA', 'hedef_uyelik.tsv')
    if os.path.exists(hy):
        ...
        if not re.search(r'(?m)^%s\t' % re.escape(a.hedef), metin):
            ...
            print('  acik tanim EKLENDI: KAPSAMLI_ARAMA/hedef_uyelik.tsv')
        else:
            print('  acik tanim zaten var: KAPSAMLI_ARAMA/hedef_uyelik.tsv')
