...
# -------------------------------------------------------------------------
# hepsini_kos.py — dort olcum asamasini bagimlilik sirasiyla (P -> K -> D -> I)
# kosar ve sabah tek dosyadan okunacak birlesik ozeti uretir.
#
# GİRDİ  : ASAMALAR tablosundaki dort betik (TEK_PROTOKOL/tek_protokol_olc.py,
#          KURTARMA/kurtarma_turu.py, KURTARMA/dogrulama_turu.py,
#          KURTARMA/kimlik_dogrulama.py) ve onlarin urettigi TSV dosyalari;
...
# ÇAĞRAN : KAPSAMLI_ARAMA.bat -> T tusu
#          (bat icinde: wsl -e python3 "KURTARMA/hepsini_kos.py" --kok .)
#
...
ASAMALAR = [
    ('P', u'TEK PROTOKOL - panelin tamami tek kuralla olculur',
     ['TEK_PROTOKOL/tek_protokol_olc.py'], 'TEK_PROTOKOL_SONUC/panel_tek_protokol.tsv',
     u'1-2 saat'),
    ('K', u'KURTARMA - esik alti satirlar dort yolla kurtarilir',
     ['KURTARMA/kurtarma_turu.py'], 'KURTARMA_SONUC/kurtarma_satirlari.tsv',
     u'1-3 saat'),
    ('D', u'DOGRULAMA - kurtarilan ciftler uc katmanla sinanir',
     ['KURTARMA/dogrulama_turu.py'], 'DOGRULAMA_SONUC/dogrulama_uc_sutun.tsv',
     u'1-4 saat'),
    ('I', u'KIMLIK DOGRULAMA - yoneticiye giden iddialar bagimsiz sinanir',
     ['KURTARMA/kimlik_dogrulama.py'], 'KIMLIK_SONUC/kimlik_iddialari.tsv',
     u'3-7 saat'),
]
...
def ozet(kok, CIKTI, durum, yaz):
    ...
    with open(yol, 'w', encoding='utf-8') as fh:
        ...
        if not I:
            ...
            if duz:
                fh.write(u'\n### Yoneticiye gonderilecek duzeltmeler\n\n')
                for r in duz:
                    fh.write(u'**%s. %s**\n\n' % (r['no'], r['iddia']))
        ...
        fh.write(u'\n---\n\n## Nereye bakmali\n\n'
                 ...
                 u'| Yoneticiye ne yazayim? | `KIMLIK_SONUC/KIMLIK_DOGRULAMA_RAPORU.md` |\n')
    yaz('  yazildi: %s' % yol)
    yaz('')
