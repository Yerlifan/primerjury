...
Toplanti kararlari WSL_betikleri/hedefler.tsv icinde numarali duruyor
...
Kosum:
    python KURTARMA/toplanti_durumu.py --kok .
Cikti:
...
def main():
    ...
    ky = os.path.join(kok, 'WSL_betikleri', 'hedefler.tsv')
    ...
    for no, ad, duzey in K:
        ...
        # duser ve hoca hangisinin sartsiz oldugunu goremez.
    ...
    with io.open(my, 'w', encoding='utf-8', newline='') as fh:
        ...
        fh.write(u'Üretim: %s\n\n' % time.strftime('%Y-%m-%d %H:%M'))
        fh.write(u'Kaynak: `WSL_betikleri/hedefler.tsv` (toplantı kararlarının '
