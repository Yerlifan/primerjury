# -*- coding: utf-8 -*-
"""KURTARMA TURU - TEK PROTOKOL kosusunda esik altinda kalan satirlari
...
kumesi = yoneticinin isaret ettigi kutu + olcumun AYNI ORGANIZMA oldugunu gosterdigi
...
#          TEK_PROTOKOL/ek_ciftler.tsv (hedef adi takma adlari),
...
# ÇAĞRAN : KAPSAMLI_ARAMA.bat -> K tusu
#          (bat icinde: wsl -e python3 "KURTARMA/kurtarma_turu.py" --kok .)
#
...
def _esik_yukle():
    """Esigi TEK KAYNAKTAN okur: KAPSAMLI_ARAMA/yapilandirma.py.
    KURTARMA/ ile KAPSAMLI_ARAMA/ kardes klasorler oldugu icin kok buradan
    ...
    from KAPSAMLI_ARAMA import yapilandirma as _y
    return _y
...
def tek_protokol_oku(kok):
    ...
    if not os.path.exists(yol):
        sys.exit('HATA: %s yok.\n      Once KAPSAMLI_ARAMA.bat -> secenek (P) kosulmalidir.' % yol)
...
# atlanirdi.
def takma_adlar(kok):
    """TEK_PROTOKOL/ek_ciftler.tsv: hedef -> uyelik_hedefi (uyelik tablosundaki ad)"""
    yol = os.path.join(kok, 'TEK_PROTOKOL', 'ek_ciftler.tsv')
    out = {}
    if not os.path.exists(yol):
        return out
    with open(yol, encoding='utf-8') as fh:
...
def yol3_yeniden_tasarim(kok, nm, hedef, uye, rakip, kons, mevcut_F, mevcut_R,
    ...
    try:
        from KAPSAMLI_ARAMA import yapilandirma as C, motor, uretec as U, geometri as G
...
# aranirlar (KAPSAMLI_ARAMA/motor.find_sites). Bir capa bulunamazsa o sinir
...
def yol5_cok_lokuslu(kok, nm, hedef, uye, rakip, kons, aday_ust=150,
    ...
    from KAPSAMLI_ARAMA import motor, uretec as U
...
def calistir(kok, aday_ust, yalniz, sifirla, tarama_ust=3000, okuma=OKUMA_TAVANI,
    ...
    from KAPSAMLI_ARAMA import numune as N, hedefler as H
    ...
    yaz('=' * 78)
    yaz('  KURTARMA TURU - esik altinda kalan satirlar')
    ...
    rc = girdi_denetle(yaz, 'K (KURTARMA)', [
...
# -------------------------------------------------------------------------
# HEDEF HEDEF KURTARMA DONGUSU. Her hedef icin sira sabittir:
...
def _tur(kok, CIKTI, KONTROL, yaz, nm, hedefler, uyelik, kons, kut, eslenik,
    ...
    rc = cikti_denetle(yaz, 'K (KURTARMA)', [
...
def raporla(CIKTI, sonuc, yaz):
    ...
    with open(yol, 'w', encoding='utf-8', newline='') as fh:
        fh.write(u'# KURTARMA TURU - her hedef icin TEK satir.\n')
...
def main():
    ...
    if not os.path.isdir(os.path.join(kok, 'KAPSAMLI_ARAMA')):
        sys.exit('HATA: %s icinde KAPSAMLI_ARAMA yok. --kok ile proje klasorunu verin.' % kok)
