...
motoru (KAPSAMLI_ARAMA/okuma_motoru.py, ispcr.py) kullaniyor. O motorda bir hata
...
# -------------------------------------------------------------------------
# mfe_katmani.py — MFEprimer 4.4.0 ikilisini (hedef disi amplikon, hairpin,
...
#          .primerqc.bin indeksleri (MFE_DB listesi), ARACLAR/mfeprimer ikilisi,
...
#          dogrulama_turu.py birlestirir.
# ÇAĞRAN : KAPSAMLI_ARAMA.bat -> D tusu (dolayli: dogrulama_turu.py bu modulu
...
MFE_ADAYLARI = [os.path.join('ARACLAR', 'mfeprimer'), 'mfeprimer']
...
# ARACLAR/mfeprimer once proje icinde aranir, sonra PATH uzerinde. Proje icindeki
...
def klad_tablosu(kok):
    """KAPSAMLI_ARAMA/hedef_klad.tsv -> {hedef: (alan, [klad jetonlari], kaynak)}"""
    y = os.path.join(kok, 'KAPSAMLI_ARAMA', 'hedef_klad.tsv')
