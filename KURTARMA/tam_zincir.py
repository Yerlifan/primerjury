...
#          altindaki dort betik) ve ../SRC/WSL/130_KRAKEN_ARAC.sh;
...
def d_tablo(kok, ayar):
    ...
    """
    y = os.path.join(ayar['src'], '0_TESLIM_HOCA', 'KRAKEN_KARSILASTIRMA.md')
    if not os.path.exists(y):
...
ASAMALAR = [
    ...
    ('I', u'KIMLIK DOGRULAMA - yoneticiye giden iddialar sinanir', u'Grup 2',
     (180, 260), False,
...
def kraken2_bul(karac, ortam):
    ...
    Ama bu projede kraken2 PATH'te DEGILDIR: the colleague's kurulumunda micromamba'nin
    ...
    cagirmaktir. 130_KRAKEN_ARAC.sh icindeki ortam_ac fonksiyonu the colleague's
...
def kraken_ortami(kok, pluspfp, vt_a, ortam=''):
    ...
    """
    src = os.path.abspath(os.path.join(kok, '..', 'SRC'))
    karac = os.path.join(src, 'WSL', '130_KRAKEN_ARAC.sh')
    sebep = []
...
def ozet_yaz(kok, CIKTI, ayar, secili, durum, kesildi):
    ...
    bakilacak = [
        ...
        (u'Kraken eşik eğrisi', '../SRC/SONUCLAR/kraken_esik_A/esik_egrisi.txt'),
        (u'Yoneticiye gidecek Kraken tablosu', '../SRC/0_TESLIM_HOCA/KRAKEN_KARSILASTIRMA.md'),
    ]
...
def main():
    ...
    if a.sifirdan:
        ...
        ali_is = os.path.join(kok, '..', 'SRC', 'SONUCLAR', 'kraken_esik_A')
        if os.path.isdir(ali_is):
            try:
                os.rename(ali_is, ali_is + '_ONCEKI_' + damga)
                tasinan.append('SRC/SONUCLAR/kraken_esik_A')
            except OSError:
                pass
