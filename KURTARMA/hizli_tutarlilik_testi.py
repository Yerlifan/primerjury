...
# -------------------------------------------------------------------------
# hizli_tutarlilik_testi.py — zincirin dort asamasini kucuk ve cevabi bilinen
...
#          altina sembolik baglarla baglanir (KAPSAMLI_ARAMA, TEK_PROTOKOL,
#          KURTARMA, REFERANS_DB, konsensus_kanonik, primer_final, "fastq files",
...
# ÇAĞRAN : KAPSAMLI_ARAMA.bat -> H tusu
#          (bat icinde: wsl -e python3 "KURTARMA/hizli_tutarlilik_testi.py" --kok .)
#
...
def calistir(kok, hizli_kok, tavan_dk, yaz):
    ...
    rc, _ = kos(yaz, 'P (TEK PROTOKOL, %d okuma)' % OKUMA,
                [py, os.path.join(kok, 'TEK_PROTOKOL', 'tek_protokol_olc.py'),
...
def sonraki_asamalar(kok, hizli_kok, tavan_dk, yaz, sonuc):
    ...
    rc, _ = kos(yaz, 'K (KURTARMA)',
                [py, os.path.join(kok, 'KURTARMA', 'kurtarma_turu.py'),
    ...
    rc, _ = kos(yaz, 'D (DOGRULAMA, yalniz yerel katman)',
                [py, os.path.join(kok, 'KURTARMA', 'dogrulama_turu.py'),
    ...
    rc, _ = kos(yaz, 'I (KIMLIK, 2 veritabani, nt yok)',
                [py, os.path.join(kok, 'KURTARMA', 'kimlik_dogrulama.py'),
...
#           D-2 duzeltmesinde sutunu yeniden adlandirmis (dogrulama_turu.py
...
def raporla(hizli_kok, sonuc, yaz, gecen_sure):
    ...
    if sonuc.get('referans_bayat'):
        ...
        yaz(u'    python KURTARMA/referans_yenile.py --kok .')
    yaz('=' * 74)
    yaz(u'  ayrinti: %s' % yol)
    if not guvenilir:
...
def main():
    ...
    for ad in ('KAPSAMLI_ARAMA', 'TEK_PROTOKOL', 'KURTARMA', 'REFERANS_DB',
               'konsensus_kanonik', 'primer_final', 'fastq files',
               'SON_ETAP_betikleri', 'MADDE123_betikleri', 'DUZELTME_betikleri',
               'WSL_betikleri', 'UYELIK_YENIDEN'):
