...
# -------------------------------------------------------------------------
# erisim_dogrulama.py — her referans veritabaninin ARAMA TARAFINDAN gercekten
...
#          kimlik_dogrulama.py icindeki VTB listesinden okunur) ve o kumelerin
...
# ÇAĞRAN : KAPSAMLI_ARAMA.bat -> E tusu
#          (bat icinde: wsl -e python3 "KURTARMA/erisim_dogrulama.py" --kok .)
#
...
def main():
    ...
    _kd_yolu = os.path.join(kok, 'KURTARMA', 'kimlik_dogrulama.py')
    _eksik = []
    if not os.path.isdir(os.path.join(kok, 'KURTARMA')):
        _eksik.append('KURTARMA klasoru')
    elif not os.path.exists(_kd_yolu):
        _eksik.append('KURTARMA/kimlik_dogrulama.py')
    if not os.path.isdir(os.path.join(kok, 'REFERANS_DB')):
        _eksik.append('REFERANS_DB klasoru')
    if _eksik:
        sys.stderr.write(
            u'HATA: %s icinde %s yok.\n'
            u'  Bu betik proje kokunden calisir. Kok, KAPSAMLI_ARAMA.bat ile ayni\n'
            u'  klasordur ve icinde KURTARMA/ ile REFERANS_DB/ bulunur.\n'
            u'  Dogru kullanim:  python3 KURTARMA/erisim_dogrulama.py --kok <proje klasoru>\n'
    ...
    sys.path.insert(0, os.path.join(kok, 'KURTARMA'))
    ...
    try:
        ...
        sys.stderr.write(
            u'HATA: kimlik_dogrulama.py yuklenemedi: %s: %s\n'
