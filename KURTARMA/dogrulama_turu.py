...
Bu betik KURTARMA turunun (secenek K) esigi gecirdigi HER YENI ya da DEGISMIS
cift icin iki kanit katmani daha kosar ve ucunu YAN YANA koyar:
  ...
  2) YEREL VERITABANI      - REFERANS_DB altindaki kumeler; mevcut kuresel
                             tarama kodu (KAPSAMLI_ARAMA/kuresel_tarama.py)
...
#          REFERANS_DB/ altindaki KUMELER listesi, ARACLAR/mfeprimer indeksleri,
...
# ÇAĞRAN : KAPSAMLI_ARAMA.bat -> D tusu
#          (bat icinde: wsl -e python3 "KURTARMA/dogrulama_turu.py" --kok . ...)
#
...
# Kaynak: panelin kendi Ta'si (Burak, 2026-08-07). MFEprimer her amplikon icin
...
=========================================
Kullanilan kod: KAPSAMLI_ARAMA/kuresel_tarama.py  (AYNEN, degistirilmedi)
...
def siparistekiler(kok, hepsi=False):
    ...
    if not os.path.exists(yol):
        sys.exit('HATA: SIPARIS_LISTESI.tsv yok.\n'
                 '      Once KAPSAMLI_ARAMA.bat -> secenek (T) kosulmalidir.')
...
def kurtarilanlar(kok):
    ...
    if not os.path.exists(yol):
        sys.exit('HATA: %s yok.\n      Once KAPSAMLI_ARAMA.bat -> secenek (K) kosulmalidir.' % yol)
    with open(yol, encoding='utf-8') as fh:
...
# mi. Mevcut kuresel tarama kodu (KAPSAMLI_ARAMA/kuresel_tarama.py) AYNEN
...
def katman1_yerel(kok, ciftler, yaz, kontrol_dizin, parc=False, kume_ust=0):
    ...
    from KAPSAMLI_ARAMA import kuresel_tarama as KT, yapilandirma as C
    ...
    try:
        sys.path.insert(0, kok)
        from KAPSAMLI_ARAMA import taksonomi as TX
        import KURTARMA.mfe_katmani as _MK
    ...
    for i in range(deneme):
        try:
            req = urllib.request.Request(
                url, veri, headers={'User-Agent': 'MicRhoBooster-primer-QC/1.0'})
...
def katman2_oto(ciftler, cikti, yaz, organizma='', bekleme=20, tur_ust=60,
    ...
    # hedef disidir. Harita dosyasi: KAPSAMLI_ARAMA/hedef_taxid.tsv
    def _ent_of(_tx):
                ...
                if not _kendi_dislandi:
                    out[ad] = dict(
                        ...
                        not_=u'Sayfa %d urun listeledi ama bu hedef icin '
                             u'KAPSAMLI_ARAMA/hedef_taxid.tsv icinde dislanacak takson '
...
def katman2_elle_girdi(ciftler, cikti, yaz, organizma=''):
    ...
    with open(s, 'w', encoding='utf-8', newline='') as fh:
        ...
        fh.write(u'#   KAPSAMLI_ARAMA.bat -> (D) -> "elle sonuc yukle" ya da\n')
        fh.write(u'#   python3 KURTARMA/dogrulama_turu.py --kok . --ncbi-yukle '
...
def raporla(cikti, satirlar, yaz):
    ...
    with open(r, 'w', encoding='utf-8') as fh:
        ...
        fh.write(u'\n## NCBI nasil tamamlanir\n\n'
                 ...
                 u'3. `KAPSAMLI_ARAMA.bat` -> (D) -> "elle sonuclari yukle" secenegini kosun.\n')
    yaz('  yazildi: %s' % r)
    yaz('')
...
def main():
    ...
    # -----------------------------------------------------------------------
    # --tumu  (2026-08-07)
    # Burak'in istegi: "hepsini ekstra o veritabaninda da arasin". --siparis
    ...
    if not os.path.exists(kyol_) or sum(
        ...
        yaz(u'  Sebep: K (KURTARMA) asamasi hic satir uretmedi, dolayisiyla')
    ...
    if _ATLANAN:
        yaz(u'  ATLANAN (primer dizisi bulunamadi - TEK_PROTOKOL ciktisinda yok): %s'
            % ', '.join(_ATLANAN))
    for c in ciftler:
    ...
    if not a.mfe_yok:
        ...
        mfe = MK.mfe_bul(kok)
        if not mfe:
            yaz(u'  MFEprimer ikilisi bulunamadi (ARACLAR/mfeprimer) - katman ATLANDI.')
        else:
            yaz(u'  ikili: %s' % mfe)
    ...
    # ve KAPSAMLI_ARAMA/hedef_klad.tsv'yi okur; ikisinden biri yoksa BOS doner
    # ve o zaman ham sayi kullanilir ama rapor bunu acikca yazar.
    klad_sonuc = {}
    try:
        import KURTARMA.mfe_katmani as _MK2
    except ImportError:
        _MK2 = None
        try:
