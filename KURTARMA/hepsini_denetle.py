...
Kosum:
    python KURTARMA/hepsini_denetle.py --kok .
    python KURTARMA/hepsini_denetle.py --kok . --agsiz     (NCBI adimlarini atla)
"""
...
def d1_harita_anahtarlari(kok, yaz):
    ...
    hy = os.path.join(kok, 'KAPSAMLI_ARAMA', 'hedef_taxid.tsv')
...
def d2_kapsama(kok, yaz, agsiz):
    bet = os.path.join(kok, 'KAPSAMLI_ARAMA', 'dislama_kapsama_denetimi.py')
    ...
    if p.returncode != 0:
        ...
        if kotu:
            bulgu(u'Dislama taksonu uyeleri KAPSAMIYOR',
                  ...
                  u'sayilir. Ayrinti: KAPSAMLI_ARAMA/dislama_kapsama_denetimi.py', BLOKE)
        else:
...
def d3_referans_bayat(kok, yaz):
    ...
    if not os.path.exists(ry):
        bulgu(u'Hizli test referans dosyasi YOK',
              ...
              u'icin: python KURTARMA/referans_yenile.py --kok .')
        return
    ...
    if bayat:
        bulgu(u'Referans BAYAT - primer cifti degismis',
              ...
              u'python KURTARMA/referans_yenile.py --kok .' % ', '.join(bayat), DIKKAT)
...
def d5_muhur_diziyi_iceriyor_mu(kok, yaz):
    ...
    """
    bakilacak = [
        ('TEK_PROTOKOL/tek_protokol_olc.py', True),
        ('KURTARMA/kurtarma_turu.py', True),
        ('KURTARMA/dogrulama_turu.py', True),
        ('KURTARMA/mfe_katmani.py', False),
...
def d6_cikti_tazeligi(kok, yaz, uretilecek=()):
    ...
    ciftler = [
        ...
        ('KAPSAMLI_ARAMA/hedef_uyelik.tsv',
...
def d7_geometri_kapisi(kok, yaz):
    ...
    if gecmemis:
        bulgu(u'Geometri kapisindan GECMEMIS cift',
              ...
              u'python KURTARMA/geometri_yenile.py --kok .' % ', '.join(gecmemis), BLOKE)
...
def d10_ad_kurali_sinavi(kok, yaz):
    ...
    Bu kural hedef disi SAYISINI belirliyor. Bozulursa hocaya giden tabloda
    "650 hedef disi" gibi sisirilmis sayilar cikar (2026-08-10'da tam olarak
    boyle oldu: gevsek kural 650 diyordu, siki kural 82).
    """
    bet = os.path.join(kok, 'KURTARMA', 'ncbi_yeniden_siniflandir.py')
    if not os.path.exists(bet):
...
def d11_siparis_dizileri(kok, yaz):
    """Hocaya/tedarikciye giden xlsx ile panelin SU ANKI dizileri ayni mi.

    2026-08-10 gece: sabah ozeti "diziler buradan kopyalanacak" diye
    MicRhoBooster_..._TESLIM.xlsx "2 Panel" sayfasini gosteriyordu; o sayfada
    ...
    # URETILEN MicRhoBooster_PANEL_*.xlsx denetlenir; o da yoksa denetim
    # atlanir. Arsivdeki dosyaya BAKILMAZ.
    _p = sorted(_glob.glob(os.path.join(kok, 'MicRhoBooster_PANEL_*.xlsx')))
    xl = _p[-1] if _p else ''
    if not os.path.exists(pk):
    ...
    if not xl or not os.path.exists(xl):
        yaz(u'  [11] siparis dizileri: uretilmis Excel yok - '
            u'python KURTARMA/excel_uret.py --kok . ile uretin')
...
def d12_kanitsiz_kosulsuz(kok, yaz):
    ...
    kosulsuz siparis hukmu verilmis. "Kosulsuz" sozcugu hocaya "bunda
    ...
    if supheli:
        bulgu(u'KOSULSUZ yazili ama kaniti eksik',
              u'\n      '.join(supheli) +
              u'\n      "Kosulsuz" hocaya "tartisilacak bir sey yok" diye okunur. '
...
def d14_bat_dosyalari(kok, yaz):
    ...
    2026-08-10: SABAH_TEK_TUS.bat ve KAPSAMLI_ARAMA.bat da LF cikti.
...
def d15_konsensus_kalintilari(kok, yaz):
    ...
    if not os.path.exists(ix):
        bulgu(u'Kanonik konsensus INDEKSI YOK',
              ...
              u'python KAPSAMLI_ARAMA/kanonik_uret.py --kok .' % ix)
        return
    gecerli = set()
    for r in _tsv(ix):
...
def d16_uyelik_kaynagi(kok, yaz):
    ...
    """
    sec = {}
    for ad, dizin, mod in (('P', 'TEK_PROTOKOL', 'tek_protokol_olc'),
                           ('K', 'KURTARMA', 'kurtarma_turu')):
...
def d17_veritabani_alfabesi(kok, yaz):
    ...
    Iki ayri tuzak, ikisi de yasanmis:
      ...
      2) kimlik_dogrulama.py iki dosyayi "BAYT BAYT AYNI (cmp ile dogrulandi)"
    ...
    if rna:
        bulgu(u'Indeksli veritabani RNA alfabesinde',
              ...
              u'indeksleyin: bash INDEKS_KUR.sh <dosya>', BLOKE)
    ...
    if bozuk:
        bulgu(u'"Ikiz" denen dosyalar artik ikiz DEGIL',
              u'\n      '.join(bozuk) +
              u'\n      kimlik_dogrulama.py bu dosyalari "bayt bayt ayni" diye '
...
def d19_uyelik_icerigi(kok, yaz):
    ...
    uyelik dosyasi kullaniyor ve bu bilerek boyle:
        KAPSAMLI_ARAMA/hedef_uyelik.tsv          - arama/tarama tarafi
