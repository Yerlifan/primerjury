# -*- coding: utf-8 -*-
"""
panel_guncelle.py - okuma motoru duzeltmesini panele isler.

  * yeni sayfa: "16 Okuma Motoru Duzeltmesi"
  * "2 Panel"  : her satira olcut etiketi + duzeltilmis degerler sutunlari + NOT
  * "1 Yoneticiye Ozet" ve "6 Karar Durumu": degisen satirlar ve uyari
  * yoneticiye_ozet TSV'si senkron

Kullanim: python panel_guncelle.py --xlsx ..\PrimerJury_PCR_Paneli_2026-08-02_TESLIM.xlsx
                                   --tsv  ..\okuma_motoru_duzeltmesi_20260802.tsv
"""
# ---------------------------------------------------------------------------
# panel_guncelle.py — okuma motoru duzeltmesinin sonuclarini teslim panelinin
...
#          alir; ayrica yoneticiye_ozet TSV'sini senkronlar. Ekrana guncellenen
...
def sayfa16(wb, rows):
    ...
    for r in [('okuma.Sonda (panelin kullandigi)', '2', '0,50%', 'tohumlu'),
              ...
              ('okuma_motoru.py (duzeltilmis)', '174', '43,50%', 'kaba kuvvetle BIREBIR')]:
    ...
    for s in [
        ...
        'KAYIPSIZLIK KANITLANDI: KAPSAMLI_ARAMA/test_motor.py duzeltilmis motoru, tohum kullanmayan ve her '
        ...
        'DOSYALAR: KAPSAMLI_ARAMA/okuma_motoru.py (motor), kaba_kuvvet.py (referans), test_motor.py (kanit), '
        'panel_olc.py (panel olcumu), bagimsiz_dogrulama.py, KOS_TAM_DERINLIK.bat',
    ]:
        yaz(ws, n, 1, s); ws.merge_cells(start_row=n, start_column=1, end_row=n, end_column=9)
    ...
    yaz(ws, n, 1, 'ALT KUME OLCUMUDUR: kutu basina en cok 3000 okuma (panelin kendi protokolu). '
                  'TAM DERINLIK dogrulamasi KAPSAMLI_ARAMA/KOS_TAM_DERINLIK.bat ile yapilacaktir; '
    ...
    for s in [
        ...
        'panelin KENDI kodu, bu oturumda degistirilmedi; (B) kaba_kuvvet.py - saf python, her pozisyon '
        'tek tek, ortak kod paylasmaz; (C) duzeltilmis okuma_motoru.py.',
        ...
        'Kosmak icin: python KAPSAMLI_ARAMA/bagimsiz_dogrulama.py --fastq "fastq files"',
    ]:
        yaz(ws, n, 1, s); ws.merge_cells(start_row=n, start_column=1, end_row=n, end_column=9)
        ws.row_dimensions[n].height = 46; n += 1
    n += 1
    ...
    for s in [
        ...
        'argumaniydi). KAPSAMLI_ARAMA/ciftler.tsv\'de hedef adlarindan ve taxid_adlari.tsv\'den yeniden '
...
def panel_sutunlari(wb, rows):
    ...
    yaz(ws, n, 3, 'OKUMA MOTORU DUZELTMESI (2026-08-02, en son madde): "Uye urun %", "Rakip maks %" ve '
                  ...
                  'Yeni sutunlar (saga bakiniz) alt kume olcumudur; tam derinlik KAPSAMLI_ARAMA\\'
