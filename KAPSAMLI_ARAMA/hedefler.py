...
#          WSL_betikleri/hedefler.tsv (karar tablosu, grup uyeligi),
#          WSL_betikleri/taxid_adlari.tsv, KAPSAMLI_ARAMA/hedef_uyelik.tsv
...
#          TEK_PROTOKOL/tek_protokol_olc.py (tus P), KURTARMA/kurtarma_turu.py
#          (tus K), KURTARMA/kimlik_dogrulama.py (tus I),
#          KURTARMA/tum_kutu_kimlikleri.py (tus G).
# ---------------------------------------------------------------------------
...
def acik_uyelik():
    """KAPSAMLI_ARAMA/hedef_uyelik.tsv - hedef adi -> uyelik. Elle duzenlenebilir."""
    m = {}
...
def konsensusler():
    ...
    if not ix or not os.path.exists(ix):
        raise RuntimeError(
            ...
            'Once uretin:  python KAPSAMLI_ARAMA/kanonik_uret.py --kok .\n'
...
def hedef_baglami(panel_satiri, uyelik=None, kons=None, kut=None):
    ...
    # UYELIK KAYNAK ONCELIGI: once KAPSAMLI_ARAMA/hedef_uyelik.tsv (elle
    # duzenlenen acik tanim), yoksa WSL_betikleri/hedefler.tsv (projenin karar
    ...
    if ad in acik:
        u = acik[ad]
        anahtar = 'KAPSAMLI_ARAMA/hedef_uyelik.tsv (%s)' % (u.get('kaynak') or '?')
    else:
        anahtar = AD_ESLEME.get(ad, ad)
        u = uyelik.get(anahtar)
        anahtar = 'WSL_betikleri/hedefler.tsv:%s' % anahtar
