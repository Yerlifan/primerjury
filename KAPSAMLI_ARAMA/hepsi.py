...
"""
# ---------------------------------------------------------------------------
# hepsi.py — yedi asamayi bagimlilik sirasiyla kosan toplu akis ve butun
#            asamalarin ciktilarini tek dosyada birlestiren ozet uretici.
#
# GIRDI  : yapilandirma.py'deki yollar; kontrol/hepsi_durum.json (hangi asama
...
#          Asamalari kendisi cagirir: kanonik_uret.py (alt surec olarak),
...
# CAGRAN : KAPSAMLI_ARAMA.bat tusu 9 (--mod hepsi -> HP.calistir) ve tusu
#          S (--mod ozet -> HP.yalniz_ozet). Ayrica yon_kapisi() fonksiyonu
#          __main__.py, panel_olcum.py, uyelik_denetimi.py ve konsensus_uret.py
...
def yon_kapisi(yaz, asama):
    ...
    # Kapi uc seyi sirayla sinar: yon.py kendi sinavini geciyor mu, kanonik
    # indeks okunabiliyor mu, indeksteki konsensuslerin hicbiri ANTISENSE degil
    # mi. Ucunden biri duserse asama baslatilmaz.
    from . import yon as Y
    from . import hedefler as H
    m = []
    try:
        h = Y.kendini_sina()
        if h:
            return False, ['yon.py kendi sinavindan GECEMEDI: ' + '; '.join(h)]
        kons = H.konsensusler()
    except Exception as e:
        return False, ['Kanonik konsensus okunamadi: %s' % e,
                       'Uretmek icin:  python3 KAPSAMLI_ARAMA/kanonik_uret.py --kok .']
    if not kons:
    ...
    if ters:
        return False, ['%d konsensus TERS yonde: %s' % (len(ters), ', '.join(ters[:5])),
                       ...
                       'Duzeltmek icin:  python3 KAPSAMLI_ARAMA/kanonik_uret.py --kok . --yeniden']
    m.append('yon kapisi [%s]: %d kutu, hepsi SENSE - GECTI' % (asama, len(kons)))
    return True, m


def kanonik_kos(yaz, sure, oncelik='ozgun'):
    """kanonik_uret.py'yi ayni surecte cagirir (ayri komut gerekmesin diye)."""
    import subprocess, sys as _sys
    betik = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'kanonik_uret.py')
    if not os.path.exists(betik):
        return False, 'kanonik_uret.py bulunamadi'
...
def ozet_yaz(durum, sure, gecen):
    ...
    A('| motor | ayni (`okuma_motoru.py`) | ayni |')
    A('')
    ...
    if not uyelik:
        ...
        if tani:
            ...
            for k, v in tani.items():
                A('| %s | %s |' % (k, v))
        A('')
        A('Ayrinti: `UYELIK_DENETIMI.md` · Duzeltme yeri: `KAPSAMLI_ARAMA/hedef_uyelik.tsv`')
    A('')
    ...
    A('4. Panel xlsx\'ine isleme: `KAPSAMLI_ARAMA/panel_guncelle.py` bunun icin '
