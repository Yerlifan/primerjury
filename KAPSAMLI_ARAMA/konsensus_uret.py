...
"""
# ---------------------------------------------------------------------------
# konsensus_uret.py — her kutunun konsensus dizisini ham fastq okumalarindan
...
#          okuma; yon.py ile yon normalizasyonu.
...
#          hepsi.py once kanonik_uret.py --oncelik yeni ile bunlari kanonik
#          klasore yon normalizasyonundan gecirerek alir.
# CAGRAN : KAPSAMLI_ARAMA.bat tusu 6 (--mod konsensus) ve tusu 9 icindeki
...
from . import motor, hedefler as H, kontrol
from . import yon
...
def calistir(yaz, sure, yalniz=None, yeniden=False):
    ...
    from . import kendini_sina as _KS
    if not _KS.yon_sinamasi(yaz):
        yaz('   YON SINAMASI DUSTU - konsensus uretimi BASLATILMADI.')
        yaz('   Once calistirin: python KAPSAMLI_ARAMA/kanonik_uret.py --kok .')
        return None
