...
KOD_NOT = {
 'KAPSAMLI_ARAMA/yapilandirma.py':
 ...
 'KAPSAMLI_ARAMA/kendini_sina.py':
...
def main():
    ...
    yaz(ws, n, 1, 'CEVAP: yon ARTIK KANONIK. Once degildi - dort ayri konsensus kumesi vardi '
                  ...
                  '(hepsi SENSE), tanim KAPSAMLI_ARAMA/yon.py.', fill=YESIL)
    ...
    for s in ['Yon hatasi gece boyunca EN AZ UC AYRI YERDE ayri ayri bulunup ayri ayri yamandi: '
              '(a) SRC tarafinda 85\'in iplik secimi, (b) tasarim tarafinda "konsensusler SILVA\'ya '
    ...
    n += 1
    NOT = {
     'consensus sequences': 'a colleague\'nin ozgun ciktisi. KARISIK. Sebep: samtools/minimap2 hatti '
    ...
    for s in ['KAPSAMLI_ARAMA/yapilandirma.py:  KONSENSUS = y(\'consensus sequences\')  '
              '-> paket KARISIK klasoru tek kaynak sayiyordu.',
              'KAPSAMLI_ARAMA/konsensus_uret.py -> _sablon_sec(): sablon "mevcut konsensus"tan '
    ...
    yaz(ws, n, 1, 'CAPRAZ KONTROL: ayni test panelin KENDI motoruyla (ispcr.amplify) tekrarlandi - '
                  ...
                  'python KAPSAMLI_ARAMA/yon_etki_testi.py --kok .')
    ws.merge_cells(start_row=n, start_column=1, end_row=n, end_column=7)
    ws.row_dimensions[n].height = 30; n += 2

    yaz(ws, n, 1, '5. KANONIK COZUM - tek kaynak, tek tanim', bold=True, fill=YESIL); n += 1
    for s in ['KANONIK YON = SENSE (referans / arti iplik). Tanim TEK YERDE: KAPSAMLI_ARAMA/yon.py. '
              ...
              'KAPSAMLI_ARAMA/kanonik_uret.py. Yaninda MANIFEST.tsv (her dosyanin kaynagi, '
    ...
    for r in sorted(kr, key=lambda x: (onem.get(x['sinif'], 9), x['betik'])):
        if r['betik'].startswith('KAPSAMLI_ARAMA/eski/'):
            continue
    ...
    for s in ['konsensus_uret.py UC yerde duzeltildi: (a) sablon kanonige cevriliyor, '
              ...
              'SIRA: (1) python KAPSAMLI_ARAMA/kanonik_uret.py --kok .   (2) KAPSAMLI_ARAMA.bat '
    ...
    for s in ['konsensus_kanonik su an referans_konsensus/konsensus kaynakli (99 kutu) + 1 oksuz '
              ...
              'WSL_betikleri/05_kume_bol.py karisik klasoru tek yon varsayarak okuyor. Panelin '
              ...
              'yon_kod_taramasi\'nda KAYNAK_BELIRSIZ isaretli betikler (cogu WSL_betikleri ve '
