...
4. UC MOTOR KARSILASTIRMASI (en onemlisi):
      kaba_kuvvet.py  (tohumsuz referans)
   == okuma_motoru.py (duzeltilmis, guvercin yuvasi tohumu)
   == numpy havuz     (bu aracin hizli yolu)
   Ayrica panelin ESKI motorunun (okuma.py/Sonda) ne kadar site kacirdigi
   olculup rapor edilir.
5. Pencere ureteci gercekten HER pozisyon x HER uzunlugu uretiyor mu.
"""
# ---------------------------------------------------------------------------
# kendini_sina.py — olcum baslamadan once motorlarin dogrulugunu sinayan kapi;
...
#          hepsi.calistir'in 1. asamasi. Yani KAPSAMLI_ARAMA.bat tuslari 1, 2,
#          3, 4, 5, 6, 7, 9 ve dogrudan tus 8 (--sina, yalniz sinama).
#
# Sinamanin 4. maddesi uc motoru karsilastirir: kaba_kuvvet.py (tohumsuz
# referans), okuma_motoru.py (guvercin yuvasi tohumu) ve numpy havuz yolu.
...
def yon_sinamasi(yaz):
    ...
    from . import okuma_motoru as OM
    ...
    try:
        from . import yon as _Y
        h = _Y.kendini_sina()
        tum &= _ok(yaz, 'yon.py kendi sinavi (5 madde)', not h, '; '.join(h)[:70])
...
def calistir(yaz):
    ...
    if geo_py:
        ...
        _ok(yaz, 'geo.py bulunamadi (karsilastirma atlandi)', True)
    ...
    try:
        ...
        tum &= _ok(yaz, 'okuma_motoru.py == kaba_kuvvet.py (%d durum)' % denenen,
        ...
        if en_kotu[1]:
            _ok(yaz, 'panelin ESKI motoru (okuma.py/Sonda) site kaciriyor', True,
