...
"""
# ---------------------------------------------------------------------------
# panel_olcum.py — paneldeki butun ciftleri tek ve ayni protokolle, tam okuma
...
#          numune.Numune(otorite=True) yani okuma_motoru.py uzerinden, ayrica
...
# CAGRAN : KAPSAMLI_ARAMA.bat tusu 4 (--mod panel-olc --tam-derinlik),
...
def calistir(yaz, sure, okuma_sayisi=0, yalniz=None, yeniden=False):
    ...
    yaz('  motor         : KAPSAMLI_ARAMA/okuma_motoru.py %s'
        % getattr(motor.okuma_motoru, '__version__', '?'))
    yaz('                  guvercin yuvasi tohumlamasi - KAYIPSIZ, kaba_kuvvet.py ile')
    yaz('                  birebir dogrulanmis. okuma.py/Sonda KULLANILMIYOR.')
    yaz('')
...
def rapor_yaz(sonuclar, panel_yolu, top_okuma, okuma_sayisi):
    ...
    A('')
    A('| hedef / kutu | Sonda (okuma.py) | kaba kuvvet | kayip |')
    A('|---|---|---|---|')
    ...
    A('- Uye/rakip kutu tanimi `KAPSAMLI_ARAMA/hedef_uyelik.tsv` dosyasindandir. '
