...
Kullanim (normalde KAPSAMLI_ARAMA.bat cagirir):
    python3 -m KAPSAMLI_ARAMA --mod tam
    python3 -m KAPSAMLI_ARAMA --mod sorunlu
    python3 -m KAPSAMLI_ARAMA --mod devam
    python3 -m KAPSAMLI_ARAMA --sina          (kendini sina, olcum yapmaz)
"""
# ---------------------------------------------------------------------------
...
# CAGRAN : KAPSAMLI_ARAMA.bat icinde "wsl -e python3 -m KAPSAMLI_ARAMA
...
#          KURTARMA/, TEK_PROTOKOL/, UYELIK_YENIDEN/ ve ALI/WSL klasorlerindeki
...
def uyelik_uyarisi(satir, taban):
    ...
    if not olculen:
        return ['!! UYARI: mevcut cift icin ayrim orani HIC olculemedi - uye kutularinin',
                ...
                '   KAPSAMLI_ARAMA/hedef_uyelik.tsv -> satir "%s"' % satir['hedef']]
    ...
    return [
        ...
        '   Once su dosyaya bakin: KAPSAMLI_ARAMA/hedef_uyelik.tsv  ->  satir "%s"' % satir['hedef'],
...
def main(argv=None):
    ...
    kontrol.hazirla()
    cizgi('=')
    yaz('  KAPSAMLI PRIMER ARAMASI - MicRhoBooster paneli')
