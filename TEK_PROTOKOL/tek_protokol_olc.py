...
# -------------------------------------------------------------------------
# tek_protokol_olc.py — paneldeki BUTUN ciftleri tek kural ve tek derinlikle
# yeniden olcer, tek bir siparis listesi uretir. Satir bazinda istisna yoktur.
#
# GİRDİ  : primer_final/ altindaki panel tablosu (KAPSAMLI_ARAMA.hedefler.
#          panel_oku ile), TEK_PROTOKOL/ek_ciftler.tsv (panelde olmayan ciftler),
...
# ÇAĞRAN : KAPSAMLI_ARAMA.bat -> P tusu
#          (bat icinde: wsl -e python3 "TEK_PROTOKOL/tek_protokol_olc.py" --kok .)
#
...
# --- ESIK TEK KAYNAKTAN: KAPSAMLI_ARAMA/yapilandirma.py -> ESIK_DCQ ---
def _esik_yukle():
...
def _sinif_yukle():
    from KAPSAMLI_ARAMA import siparis_siniflari as _s
    return _s
...
# Tek kaynak: KAPSAMLI_ARAMA/yapilandirma.py -> ESIK_DCQ.
...
# --------------------------------------------------------------- girdiler
# Proje kokunu dogrular. KAPSAMLI_ARAMA klasoru yoksa olcum modulleri ithal
...
def kok_bul(arg):
    ...
    if not os.path.isdir(os.path.join(kok, 'KAPSAMLI_ARAMA')):
        sys.exit('HATA: %s icinde KAPSAMLI_ARAMA klasoru yok. --kok ile proje '
                 'klasorunu verin.' % kok)
    return kok
...
def ek_ciftler_oku(kok):
    """TEK_PROTOKOL/ek_ciftler.tsv - panel TSV'sinde OLMAYAN ciftler.
...
def calistir(kok, okuma_tavani, karisik_kural, yalniz=None, sifirla=False):
    ...
    from KAPSAMLI_ARAMA import yapilandirma as C, motor, numune as N, hedefler as H
    ...
    if not uy_yol:
        sys.exit('HATA: uyelik_yeniden_turetme_uyelik_*.tsv bulunamadi.\n'
                 '      Once KAPSAMLI_ARAMA.bat -> secenek U kosulmalidir.')
