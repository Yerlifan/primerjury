...
#
# GİRDİ  : primer_final/ altindaki panel tablosu (screening.hedefler.
...
# ÇAĞRAN : screening.bat -> P tusu
#          (bat icinde: wsl -e python3 "protocol/tek_protokol_olc.py" --kok .)
#
...
# --- ESIK TEK KAYNAKTAN: screening/yapilandirma.py -> ESIK_DCQ ---
def _esik_yukle():
...
def _sinif_yukle():
    from screening import siparis_siniflari as _s
    return _s
...
# Tek kaynak: screening/yapilandirma.py -> ESIK_DCQ.
...
def calistir(kok, okuma_tavani, karisik_kural, yalniz=None, sifirla=False):
    ...
    from screening import yapilandirma as C, motor, numune as N, hedefler as H
    ...
    if not uy_yol:
        sys.exit('HATA: uyelik_yeniden_turetme_uyelik_*.tsv bulunamadi.\n'
                 '      Once screening.bat -> secenek U kosulmalidir.')
