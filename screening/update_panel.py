...
def sayfa16(wb, rows):
    ...
    for s in [
        ...
        'MEMBER SETS: the panel\'s original member taxon sets were never stored in any script (they were a command line argument). They were rebuilt from the target names in screening/pairs.tsv and from taxid_names.tsv. On rows marked "PANELLE_TUTUYOR" the rebuilt set reproduces the member % range the panel published; on rows marked "YENIDEN_KURULDU" it does not, and on those rows the old vs new DIFFERENCE is valid (both engines ran on the same bins) but the absolute values may not match the panel\'s original set definition. The Proteiniphilum (row 10) and Bacteroidales (row 12) sets in particular should be checked.',
