...
# CAGRAN : screening.bat tusu 9 (--mod hepsi -> HP.calistir) ve tusu
...
def yon_kapisi(yaz, asama):
    ...
    try:
        ...
        return False, ['Kanonik konsensus okunamadi: %s' % e,
                       'Uretmek icin:  python3 screening/kanonik_uret.py --kok .']
    if not kons:
    ...
    if ters:
        return False, ['%d konsensus TERS yonde: %s' % (len(ters), ', '.join(ters[:5])),
                       ...
                       'Duzeltmek icin:  python3 screening/kanonik_uret.py --kok . --yeniden']
...
def ozet_yaz(durum, sure, gecen):
    ...
    A('4. Panel xlsx\'ine isleme: `screening/panel_guncelle.py` bunun icin '
