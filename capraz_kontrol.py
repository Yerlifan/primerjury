# -*- coding: utf-8 -*-
u"""
CAPRAZ KONTROL - MicRhoBooster paneli icin BAGIMSIZ, SALT OKUNUR denetci.
===========================================================================
...
NE YAPMAZ  --  BU ONEMLI
    ...
    KAPSAMLI_ARAMA.bat, SABAH_TEK_TUS.bat, KURTARMA/tek_tus.py, KONSENSUS_YENIDEN/.
...
KULLANIM
    python capraz_kontrol.py --kok .
    python capraz_kontrol.py --kok . --moduller 2,3,5      (yalniz secilenler)
    python capraz_kontrol.py --kok . --m1-kip tam          (agir kimlik taramasi)
    python capraz_kontrol.py --kok . --kendini-sina        (bozuk girdi sinamasi)
...
# CIDDIYET DUZEYLERI
# ===========================================================================
# KRITIK : siparisi ya da hocaya verilecek sayiyi DOGRUDAN yanlis yapar
...
def _kod_govdesi(metin):
    ...
    KAPSAMLI_ARAMA/yon_kod_taramasi.kod_govdesi() ile ayni ISI yapar ama olcut
    ...
    # (KAPSAMLI_ARAMA/hepsi.py ozgun 222, kirpilmis govdede 183).
    out = []
...
# kullaniciya gosterilen bir mesajdir (olculdu: KAPSAMLI_ARAMA/hepsi.py:183
...
def modul_yukle(yol, ad):
    ...
    KURTARMA/kimlik_dogrulama.py ve tum_kutu_kimlikleri.py'nin mantigi burada
...
class Kaynaklar(object):
    ...
    DOKUNULMAZ = (u'KAPSAMLI_ARAMA.bat', u'SABAH_TEK_TUS.bat',
                  os.path.join(u'KURTARMA', u'tek_tus.py'), u'KONSENSUS_YENIDEN')
    ...
    def __init__(self, kok):
        ...
        # --- panel tanimi
        self.ciftler = y('KAPSAMLI_ARAMA', 'ciftler.tsv')
        self.hedef_uyelik = y('KAPSAMLI_ARAMA', 'hedef_uyelik.tsv')
        self.hedef_klad = y('KAPSAMLI_ARAMA', 'hedef_klad.tsv')
        self.hedefler_wsl = y('WSL_betikleri', 'hedefler.tsv')
        self.takson_esleme = y('KAPSAMLI_ARAMA', 'hedef_takson_esleme.py')
        # --- excel
        ...
        _ad = sorted(_glob.glob(y('MicRhoBooster_PANEL_*.xlsx')))
        ...
        self.kod_klasorleri = [y('KAPSAMLI_ARAMA'), y('KURTARMA'), y('WSL_betikleri')]
        # --- ice aktarilacak mantik
        self.kimlik_dogrulama = y('KURTARMA', 'kimlik_dogrulama.py')
        self.tum_kutu = y('KURTARMA', 'tum_kutu_kimlikleri.py')
...
def modul_1_kimlik(kay, rap, kn, kip=u'hizli', yalniz=None, tavan=0):
    ...
    # --- karar mantigini KURTARMA'dan ice aktar (yeniden YAZILMAZ)
    ...
    # etkiler; KURTARMA/kimlik_dogrulama.py dosyasi DEGISMEZ.
    ...
    for kb in kutular:
        ...
        # --- ad degisti mi (hocaya gosterilecek asil sutun)
        ...
        if ad_degisti == u'EVET':
            rap.ekle(M, u'M1-AD-DEGISTI', CIDDI,
                     ...
                     krapor or kb['yol'],
                     u'Hocaya gosterilecek asil satir budur: Kraken etiketi olcumle degisti.')
...
def modul_3_uyelik(kay, rap):
    ...
    if ciftler is None:
        rap.atla(M, u'M3-CIFTLER', u'KAPSAMLI_ARAMA/ciftler.tsv okunabilmeli',
                 u'dosya yok', kay.ciftler)
        return
    if uyelik is None:
        rap.atla(M, u'M3-UYELIK', u'KAPSAMLI_ARAMA/hedef_uyelik.tsv okunabilmeli',
...
def modul_5_desenler(kay, rap):
    ...
    if ind is None:
        ...
        # WSL_betikleri/03_primer_aday_uret.py ve 04_grup_primer.py, ikisi de
        # yolu CLI argumani olarak alir, gomulu yol tasimaz).
        # Ayni kontrolun DOGRU surumu projede zaten vardi:
        # KAPSAMLI_ARAMA/yon_kod_taramasi.kod_govdesi(). Iki tarayici ayni
    ...
    for bolum, istek, durum in istekler:
        ...
        if kategoriler:
            ...
            rap.ekle(M, u'M7-KATEGORISIZ-BOSLUK', CIDDI,
                     ...
                     u'Kategorisiz bir bosluk, hocaya "neden olmadi" sorusuna '
                     u'cevap veremez.')
    ...
    if ciftler is not None and karar_hedefleri:
        ...
        for h in baglanmayan:
            ...
            rap.ekle(M, u'M7-PANELDE-FAZLA', UYARI,
                     ...
                     u'Hocaya "bu neden panelde" diye sorulursa dayanak gosterilemez.')
...
def raporla(kay, rap, cikti, kosulan, sureler):
    ...
    # ---------------- M1 kimlik tablosu ayri TSV (hocaya gidecek asil tablo)
    kimlik_yol = None
    ...
    L.append(u'')
    L.append(u'MicRhoBooster qPCR paneli - bagimsiz, salt okunur denetim.')
    L.append(u'')
    L.append(u'| | |')
    L.append(u'|---|---|')
    L.append(u'| Tarih | %s |' % time.strftime('%Y-%m-%d %H:%M'))
    L.append(u'| Betik | capraz_kontrol.py %s |' % VERSIYON)
    ...
    if satirlar:
        ...
        L.append(u'**Adi DEGISEN kutular (%d)** - hocaya gosterilecek asil tablo:'
                 % len(degisen))
        L.append(u'')
        if degisen:
...
def _sinama_kok_kur(gecici, kay):
    ...
    os.makedirs(os.path.join(gecici, 'KAPSAMLI_ARAMA'))
    os.makedirs(os.path.join(gecici, 'KURTARMA'))
    ...
    d(y('KAPSAMLI_ARAMA', 'ciftler.tsv'),
    ...
    d(y('KAPSAMLI_ARAMA', 'hedef_uyelik.tsv'),
      u'hedef\tuye_taxid\tharic\tkaynak\tnot\n'
      u'Hedef_A\t111,222\t999\tPANEL\t-\n'
      u'Hedef_B\t333\t\tPANEL\t-\n')
    d(y('KAPSAMLI_ARAMA', 'hedef_takson_esleme.py'),
...
def kendini_sina(kay, cikti):
    ...
    def kur_m3(g):
        ...
        yol = os.path.join(g, 'KAPSAMLI_ARAMA', 'ciftler.tsv')
...
def main():
    p = argparse.ArgumentParser(
        description=u'MicRhoBooster paneli - bagimsiz, salt okunur capraz kontrol')
