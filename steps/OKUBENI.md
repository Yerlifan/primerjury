...
veritabanı çalıştırılabilir. kaynak calisma da pluspf16'yı bellek eşlemesiyle
...
`PROJECT\fastq files` klasöründeki dosyalar ham okuma değil, her
örnekten en bol beş taksonun çıkarılmış hali; bu degerlendiricinın istediği biçim.
...
## kaynak calismanin konsensüs betiğinde sessiz bir hata var
...
bash reclassify_kraken2.sh \
     ...
     --out "/path/to/your/project/kraken_yeni" \
     --only-benchmark
```
...
```bash
bash analyze_ambiguous_bases.sh \
     --pt  "/path/to/your/project" \
     --out "/path/to/your/project/N_analizi"
```
...
uzunluğu. a colleague `database300mers.kmer_distrib` kullanmış, ancak `kraken results`
...
kaynak calismanin `bracken_species.sh` betiğinde iki hata var, düzeltilmiş sürümü
...
- `diger\` altında `tools\linux-x64` (Bun çalışma zamanı, primer aracı değil),
...
okuyamaz. `tools\mfeprimer` (Linux ELF, modül sürümü
...
# Ek: generate_primer_candidates.py, toplantı kararlarının uygulanması
...
# Ek: design_group_primers.py, Karar 2, 3 ve 4 için çok üyeli motor
...
Önce adlandırma: konsensüs klasörlerindeki `F1-*` kaynak calismanin adlandırmasıdır ve
...
`split_clusters.py` betiği kümeyi 04 ile deniyor, geçerli çift çıkmazsa 04'un
...
Daha önce konsensüs uzunluklarına bakıp "ekran görüntüsü doğru, kaynak calismanin klasör
...
# Ek: anchored_reference_consensus.sh
...
Sıra: `06` ile konsensüsleri yeniden kur, `analyze_ambiguous_bases.sh` ile maske üret,
sonra `design_group_primers.py` ve `split_clusters.py` ile tasarıma dön.
