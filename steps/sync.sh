...
#   bash sync.sh --verify     # compare against the recorded manifest
#
...
uret() {
  ...
  echo "# produced: $(date '+%Y-%m-%d %H:%M:%S')"
  echo "# The SCRIPT section must be EXACTLY THE SAME on both sides."
  echo "[BETIK]"
  # EVERY script in this directory, not a hand written list.
  #
  # MEASURED: the list used to name the step scripts by the pattern they were
  # called by at the time ('[0-9][0-9]_*.py', 'bol_*.sh'). When those files were
  # renamed, the patterns went on matching NOTHING and the section quietly
  # covered 9 of 37 scripts. A changed script would then pass the check in
  # silence, which is the one thing this script exists to prevent. A hand
...
if [ "$DOGRULA" = 1 ]; then
  ...
  FARK=0
  # the ones present on one side only
  ...
  join -j2 -o 0,1.1,2.1 "$ESKI_T" "$YENI_T" 2>/dev/null \
    | awk '$2!=$3{printf "  DIFFERENT %s\n     manifest: %s\n     disk    : %s\n",$1,substr($2,1,16),substr($3,1,16); n++} END{if(!n) print "  every file matches the manifest"}'
...
echo "For the next check: bash sync.sh --verify"
