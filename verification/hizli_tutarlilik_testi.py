...
# ÇAĞRAN : screening.bat -> H tusu
#          (bat icinde: wsl -e python3 "verification/hizli_tutarlilik_testi.py" --kok .)
#
...
def calistir(kok, hizli_kok, tavan_dk, yaz):
    ...
    rc, _ = kos(yaz, 'P (TEK PROTOKOL, %d okuma)' % OKUMA,
                [py, os.path.join(kok, 'protocol', 'tek_protokol_olc.py'),
...
def sonraki_asamalar(kok, hizli_kok, tavan_dk, yaz, sonuc):
    ...
    rc, _ = kos(yaz, 'K (verification)',
                [py, os.path.join(kok, 'verification', 'kurtarma_turu.py'),
    ...
    rc, _ = kos(yaz, 'D (DOGRULAMA, yalniz yerel katman)',
                [py, os.path.join(kok, 'verification', 'dogrulama_turu.py'),
    ...
    rc, _ = kos(yaz, 'I (KIMLIK, 2 veritabani, nt yok)',
                [py, os.path.join(kok, 'verification', 'kimlik_dogrulama.py'),
...
def raporla(hizli_kok, sonuc, yaz, gecen_sure):
    ...
    if sonuc.get('referans_bayat'):
        ...
        yaz(u'    python verification/referans_yenile.py --kok .')
    yaz('=' * 74)
    yaz(u'  ayrinti: %s' % yol)
    if not guvenilir:
