...
Kosum:
    python verification/hepsini_denetle.py --kok .
    python verification/hepsini_denetle.py --kok . --agsiz     (NCBI adimlarini atla)
"""
...
def d2_kapsama(kok, yaz, agsiz):
    bet = os.path.join(kok, 'screening', 'dislama_kapsama_denetimi.py')
    ...
    if p.returncode != 0:
        ...
        if kotu:
            bulgu(u'Dislama taksonu uyeleri KAPSAMIYOR',
                  ...
                  u'sayilir. Ayrinti: screening/dislama_kapsama_denetimi.py', BLOKE)
        else:
...
def d3_referans_bayat(kok, yaz):
    ...
    if not os.path.exists(ry):
        bulgu(u'Hizli test referans dosyasi YOK',
              ...
              u'icin: python verification/referans_yenile.py --kok .')
        return
    ...
    if bayat:
        bulgu(u'Referans BAYAT - primer cifti degismis',
              ...
              u'python verification/referans_yenile.py --kok .' % ', '.join(bayat), DIKKAT)
...
def d5_muhur_diziyi_iceriyor_mu(kok, yaz):
    ...
    """
    bakilacak = [
        ('protocol/tek_protokol_olc.py', True),
...
def d7_geometri_kapisi(kok, yaz):
    ...
    if gecmemis:
        bulgu(u'Geometri kapisindan GECMEMIS cift',
              ...
              u'python verification/geometri_yenile.py --kok .' % ', '.join(gecmemis), BLOKE)
...
def d10_ad_kurali_sinavi(kok, yaz):
    ...
    """
    bet = os.path.join(kok, 'verification', 'ncbi_yeniden_siniflandir.py')
    if not os.path.exists(bet):
...
def d11_siparis_dizileri(kok, yaz):
    ...
    if not xl or not os.path.exists(xl):
        yaz(u'  [11] siparis dizileri: uretilmis Excel yok - '
            u'python verification/excel_uret.py --kok . ile uretin')
...
def d14_bat_dosyalari(kok, yaz):
    ...
    2026-08-10: SABAH_TEK_TUS.bat ve screening.bat da LF cikti.
...
def d15_konsensus_kalintilari(kok, yaz):
    ...
    if not os.path.exists(ix):
        bulgu(u'Kanonik konsensus INDEKSI YOK',
              ...
              u'python screening/kanonik_uret.py --kok .' % ix)
        return
    gecerli = set()
    for r in _tsv(ix):
...
def d19_uyelik_icerigi(kok, yaz):
    ...
    kod = (
        ...
        'sys.path.insert(0,%r)\n'
        'from screening import hedefler as H\n'
