...
CAGIRAN : SABAH_TEK_TUS.bat  ve  screening.bat -> B tusu
...
BU DOSYA NEDEN VAR
------------------
Var olan A tusu (verification/tam_zincir.py) on asama kosuyor ama:
...
#    kraken       True: SRC klasorundeki araca ihtiyac duyar
...
#             dosyasindan alir (screening.bat satir 1092 de bunu arar).
#    P -> K : kurtarma_turu.py girdisi TEK_PROTOKOL_SONUC/panel_tek_protokol.tsv
#             (screening.bat satir 946 bunu on kosul olarak dogruluyor).
...
#             ayni kutu iki kez taranmaz (screening.bat satir 686).
...
def ASAMALAR(ayar):
    ...
    def d_argv(kok, a):
        arg = [os.path.join('verification', 'dogrulama_turu.py'), '--kok', '.']
    ...
    L = [
        ...
        dict(kod='N', ad=u'DENETIM - tablolar, referanslar ve muhurler her kosuda bakilir',
             grup=u'Grup 4', betik='verification/hepsini_denetle.py',
             argv=lambda kok, a: [_py(os.path.join('verification', 'hepsini_denetle.py'),
                                      '--kok', '.')],
             girdi=['verification/hepsini_denetle.py',
        ...
        dict(kod='H', ad=u'HIZLI TUTARLILIK TESTI - uzun kosudan ONCE gerileme kapisi',
             grup=u'Grup 4', betik='verification/hizli_tutarlilik_testi.py',
             argv=lambda kok, a: [_py(os.path.join('verification', 'hizli_tutarlilik_testi.py'),
                                      '--kok', '.')],
             girdi=['verification/hizli_tutarlilik_testi.py',
        ...
        dict(kod='E', ad=u'VERITABANI ERISIM DOGRULAMASI - her VT gercekten okunuyor mu',
             grup=u'Grup 2', betik='verification/erisim_dogrulama.py',
             argv=lambda kok, a: [_py(os.path.join('verification', 'erisim_dogrulama.py'),
                                      '--kok', '.')],
             girdi=['verification/erisim_dogrulama.py'],
        ...
        dict(kod='P', ad=u'TEK PROTOKOL - panelin tamami TEK kuralla olculur',
             grup=u'Grup 1', betik='protocol/tek_protokol_olc.py',
             argv=lambda kok, a: [_py(os.path.join('protocol', 'tek_protokol_olc.py'),
                                      '--kok', '.')],
             # 2026-08-09: ciftler.tsv eklendi, gerekce D asamasindaki notta.
             girdi=['protocol/tek_protokol_olc.py', 'primer_final',
             ...
             bagimli=['U'],
             sure_sn=36.0,
             kaynak=u'verification/tam_zincir.py yorumu ("Olculen: P 36 sn"); '
        ...
        dict(kod='K', ad=u'verification - esik alti satirlar dort yolla kurtarilir',
             grup=u'Grup 1', betik='verification/kurtarma_turu.py',
             argv=lambda kok, a: [_py(os.path.join('verification', 'kurtarma_turu.py'),
                                      '--kok', '.')],
             # 2026-08-09: ciftler.tsv eklendi, gerekce D asamasindaki notta.
             girdi=['verification/kurtarma_turu.py',
                    'screening/ciftler.tsv',
                    'TEK_PROTOKOL_SONUC/panel_tek_protokol.tsv'],
             cikti=['KURTARMA_SONUC/kurtarma_satirlari.tsv'], bagimli=['P'],
             sure_sn=300.0, kaynak=u'verification/tam_zincir.py yorumu ("K 5 dk")',
        ...
        dict(kod='D', ad=u'DOGRULAMA - paneldeki ciftler dort kanit katmaniyla sinanir',
             grup=u'Grup 1', betik='verification/dogrulama_turu.py',
             argv=d_argv,
             ...
             girdi=['verification/dogrulama_turu.py', 'verification/mfe_katmani.py',
        ...
        dict(kod='I', ad=u'KIMLIK DOGRULAMA - rapora giren iddialar bagimsiz sinanir',
             grup=u'Grup 2', betik='verification/kimlik_dogrulama.py',
             argv=lambda kok, a: [_py(os.path.join('verification', 'kimlik_dogrulama.py'),
                                      '--kok', '.')],
             girdi=['verification/kimlik_dogrulama.py'],
        ...
        dict(kod='G', ad=u'TUM KUTU KIMLIKLERI - panele giren HER kutu dogrulanir',
             grup=u'Grup 2', betik='verification/tum_kutu_kimlikleri.py',
             argv=lambda kok, a: [_py(os.path.join('verification', 'tum_kutu_kimlikleri.py'),
                                      '--kok', '.', '--nt', 'yok')],
             girdi=['verification/tum_kutu_kimlikleri.py'],
...
def kraken_bul(kok):
    ...
    if os.path.exists(aday):
        return aday, u''
    return None, (u'SRC klasoru bulunamadi (aranan: %s). W, X ve Z ATLANIR; '
