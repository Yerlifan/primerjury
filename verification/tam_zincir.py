...
"""
# ---------------------------------------------------------------------------
# tam_zincir.py - screening.bat menusundeki A tusunun motoru.
#
# GIRDI  : proje kokundeki asama betikleri (screening paketi, verification
#          altindaki dort betik) ve tools/kraken_tool.sh;
...
# CAGRAN : screening.bat -> A tusu
#          (bat icinde: wsl -e python3 "verification/tam_zincir.py" --kok . --onayla)
#
...
def d_tablo(kok, ayar):
    ...
    """
    y = os.path.join(ayar['src'], '0_TESLIM_RAPOR', 'KRAKEN_KARSILASTIRMA.md')
    if not os.path.exists(y):
...
ASAMALAR = [
    ...
    ('H', u'HIZLI TUTARLILIK TESTI - kucuk alt kumede zincir butun mu', u'Grup 4',
     (25, 40), False,
     lambda kok, a: [_py(kok, os.path.join('verification', 'hizli_tutarlilik_testi.py'),
                         '--kok', '.')],
     d_hizli_test),
    ...
    ('I', u'KIMLIK DOGRULAMA - rapora giren iddialar sinanir', u'Grup 2',
     (180, 260), False,
     lambda kok, a: [_py(kok, os.path.join('verification', 'kimlik_dogrulama.py'),
                         '--kok', '.')],
    ...
    ('G', u'TUM KUTU KIMLIKLERI - panele giren her kutu dogrulanir', u'Grup 2',
     (260, 350), False,
     lambda kok, a: [_py(kok, os.path.join('verification', 'tum_kutu_kimlikleri.py'),
                         '--kok', '.')],
    ...
    ('T', u'TAM OLCUM - P, K, D ve I bagimlilik sirasiyla', u'Grup 1',
     (30, 90), False,
     lambda kok, a: [_py(kok, os.path.join('verification', 'hepsini_kos.py'), '--kok', '.')],
...
def kraken2_bul(karac, ortam):
    ...
    Ama bu projede kraken2 PATH'te DEGILDIR: kaynak calismanin kurulumunda micromamba'nin
...
def kraken_ortami(kok, pluspfp, vt_a, ortam=''):
    ...
    """
    src = os.path.abspath(os.path.join(kok, '..', 'tools'))
    karac = os.path.join(src, 'WSL', 'kraken_tool.sh')
    sebep = []
    ...
    return {
        'src': src,
        'karac': karac,
        'kraken_is': os.path.join(src, 'SONUCLAR', 'kraken_esik_A'),
...
def plan_yaz(yaz, secili, ayar, durum):
    ...
    if not ayar['kraken_var']:
        ...
        yaz(u'      python3 verification/tam_zincir.py --kok . --yalniz W,X,Z,S --onayla')
...
def ozet_yaz(kok, CIKTI, ayar, secili, durum, kesildi):
    ...
    if atlanan:
        ...
        if not ayar['kraken_var']:
            ...
            A(u'```\npython3 verification/tam_zincir.py --kok . --yalniz W,X,Z,S --onayla\n```\n')
...
def main():
    ...
    p.add_argument('--plan', action='store_true',
                   help=u'YALNIZ plani basar ve cikar; hicbir sey kosulmaz. '
                        u'screening.bat once bunu cagirir, onayi kendi alir, '
    ...
    if not os.path.isdir(os.path.join(kok, 'screening')):
        sys.stderr.write(
            ...
            u'calisir; kok, screening.bat ile ayni klasordur.\n' % kok)
        return 1
