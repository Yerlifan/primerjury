...
# SRC KLASORU OTOMATIK BULUNUR, SABIT YAZILMAZ.
# Bu betik tools/WSL icinde duruyor; bir ust klasor SRC'dir. BASH_SOURCE
# calisan dosyanin gercek yolu oldugu icin bu tahmin degil olcumdur.
# Kurulum tasindiysa disaridan bastirilabilir:  SRC=/tam/yol/SRC bash <betik>
_BETIK_DIZIN="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SRC="${SRC:-$(cd "$_BETIK_DIZIN/.." && pwd)}"
if [ ! -d "$tools/0_TESLIM_RAPOR" ]; then
  echo "HATA: SRC klasoru dogrulanamadi ('$SRC' icinde 0_TESLIM_RAPOR yok)."
  echo "  Betik konumu: $_BETIK_DIZIN"
  echo "  Elle vermek icin:  SRC=/tam/yol/SRC bash $0"
  exit 1
fi
...
echo
python3 "$BURASI/ozgun Kraken ozet betigi" --is "$IS" --src "$SRC"
