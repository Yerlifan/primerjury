...
def hizalama_oku(src):
    ...
    """
    adaylar = [os.path.join(src, "0_TESLIM_RAPOR", "kimlik_sonuc.csv"),
               os.path.join(src, "VALIDASYON_v2", "primerler", "PIPELINE_TEMIZ",
...
# ------------------------------------------------------------------ tablo
def tablo_kur(src, is_a, ad_a, is_b, ad_b, esik):
    ISIMLER = isimleri_oku(src)
    hiz, hiz_yol = hizalama_oku(src)
    ...
    for s in satirlar:
        ...
        isaret = {"a) PlusPFP BIZI DOGRULUYOR": "**dogruluyor**",
                  ...
                  "c) PlusPFP UCUNCU BIR SEY DIYOR": "**ayrisiyor**"}.get(
                      s["senaryo"], s["senaryo"])
        g.append(f"| {kisalt(s['src'],34)} | {e} | {gg} | {h} | {isaret} |")
    g.append("")
    return "\n".join(g)
...
def yorum(satirlar, bilgi):
    ...
    if not ayr:
        g.append("Yok.")
    for s in ayr:
        g.append(f"- **{s['src']}**  esik: {s['esik_ad'] or 'karar yok'}  |  "
...
def selftest():
    ...
    s_a = dict(genis_ad="Petriella musispora", src="Trichoderma asperellum", hiz_ad="Petriella")
    K("senaryo a: PlusPFP bizi dogruluyor",
      _senaryo(s_a), "a) PlusPFP BIZI DOGRULUYOR")
    s_b = dict(genis_ad="Trichoderma asperellum", src="Trichoderma asperellum", hiz_ad="Petriella")
    K("senaryo b: PlusPFP eski etiketi tekrarliyor",
      _senaryo(s_b), "b) PlusPFP ESKI ETIKETI TEKRARLIYOR")
    s_c = dict(genis_ad="Fusarium oxysporum", src="Trichoderma asperellum", hiz_ad="Petriella")
    K("senaryo c: PlusPFP ucuncu bir sey diyor",
      _senaryo(s_c), "c) PlusPFP UCUNCU BIR SEY DIYOR")
    s_d = dict(genis_ad="", src="Trichoderma asperellum", hiz_ad="Petriella")
...
def _senaryo(s):
    ...
    g, a, h = s["genis_ad"], s["src"], s["hiz_ad"]
...
def main():
    ...
    satirlar, bilgi = tablo_kur(a.src, a.is_a, a.ad_a,
    ...
    cikti = os.path.join(a.src, "0_TESLIM_RAPOR") if a.src else "."
