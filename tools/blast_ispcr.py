...
Calistirma:
  python3 blast_ispcr.py --src /mnt/c/Users/user/Desktop/SRC
  python3 blast_ispcr.py --selftest      (butun kurallarin bilinen cevapli sinavi)
  python3 blast_ispcr.py --src ... --blast   (blastn ile ikinci gorus, yavas)
"""
...
# ------------------------------------------------------------------ veri yukleme
def okumalari_yukle(src, okuma_basina=300, en_az_uzunluk=300):
    """Her taksondan sabit sayida okuma alir. Ayni tohum kullanildigi icin tekrarlanabilir."""
    dosyalar = defaultdict(list)
    for p in glob.glob(f"{src}/SONUCLAR/fastq files/*/*reads_*.fastq"):
...
def main():
    ...
    ap.add_argument("--src"); ap.add_argument("--primerler", nargs="*", default=[])
    ...
    if not a.src: ap.error("--src gerekli")

    zaman = time.strftime("%Y%m%d_%H%M%S")
    cikti = a.out or os.path.join(a.src, "0_TESLIM_RAPOR", "ICPCR_" + zaman)
    ...
    primer_yollari = [p if os.path.isabs(p) else os.path.join(a.src, p) for p in a.primerler]
    if not primer_yollari:
        primer_yollari = [os.path.join(a.src, "0_TESLIM_RAPOR", f) for f in
    ...
    veri = okumalari_yukle(a.src, a.okuma)
